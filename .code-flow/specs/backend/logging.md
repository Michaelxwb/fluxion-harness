...
## Guidance
- [framework/observability] `request_id` 全链：`ContextVar`（默认 `"-"`）+ 中间件读 `X-Request-ID` 或生成 + 日志 payload 携带 + 响应 Envelope 回显
- [adapters] 关键业务写操作同事务记 audit event：`action/entity_type/entity_id/request_id/after_ref/details`，`details` 只存 key 摘要（release_id/content_hash），禁存完整 payload
