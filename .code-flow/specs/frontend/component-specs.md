...
## Guidance
- [console] 页面=容器：只组装 hook + service + 列定义，外层包 `PageContainer`，列表区用 `StandardListPage` 壳（`Card` + `ListToolbar` + `Table` + `ListFooter`），不手写列表骨架
- Props 必须类型化（TS interface / PropTypes / defineProps），可选项给默认值
- [package.json + docs/前端工程规范] 唯一通用 UI 库为 Semi Design（`@douyinfe/semi-ui`），禁止引入第二套通用组件库；普通列表页默认用统一 `StandardListPage` 骨架
