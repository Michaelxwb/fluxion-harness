...
## Guidance
- [apps/framework/adapters] 分层：`apps/<app>/{main,routes,dependencies}` 只做装配+路由转发+ DTO 转换；领域/用例放 `framework/<能力包>/{domain,execution,contracts}`；持久化放 `adapters/postgres/{models,*_repository,session}`，禁止跨层倒置依赖
- 入口文件（`main.*`）只做框架装配，不写业务代码
- [framework/settings.py] 配置收口 `Settings(BaseSettings)`（`.env`），生产代码禁止直接读 `os.environ`（测试用 `TEST_DATABASE_URL` 覆盖为唯一例外）
- 常量集中放 `constants/`，按业务模块拆分（`constants/order.py`、`constants/user.py`），禁止在业务代码中散落硬编码字面量（魔法数字 / 字符串 / 状态码）
- 数据访问层独立放 `adapters/postgres/*_repository.py`，业务层依赖 Repository 抽象，不直接依赖 ORM
- 新增一级目录必须同步更新导航地图与 `__init__` / `index` 索引

## Patterns
- 测试目录按测试层级分 `tests/{unit,architecture,integration}`，不对源码目录做镜像同构
- 模块按能力包拆分（如 `framework/domain/`、`framework/execution/`），目录深度建议 ≤ 3 层
