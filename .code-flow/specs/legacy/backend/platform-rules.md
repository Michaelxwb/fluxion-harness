...
- [framework/web + Makefile] 普通 JSON API 必须走统一 Envelope（`ok()` → `{code,message,data,request_id,timestamp}`）与 `AppError` 异常管线；构建/测试命令走 uv（`make test` / `make lint`），Gateway/Console 例外见各自目录说明
...
- 错误码常量与所属契约同模块定义（如 `contracts/resource_scope.py` 内 `SCOPE_INVALID`），错误码全局唯一，禁止重复
- [framework/web] 错误码 `RESOURCE_ACTION` UPPER_SNAKE，就近定义在 `contracts/`（如 `SCOPE_INVALID`）；status 习惯：404 资源缺失 / 409 冲突或前置缺失 / 422 校验失败 / 500 兜底 `INTERNAL_ERROR`；业务优先直接 `AppError(code=...)`，`NotFoundError/ConflictError` 子类少用
