...
>
> **归属**列声明该场景由谁闭合，只能填 `本模块` 或 `后置 → 模块 NN`：
>
> - `本模块` —— 场景的全部边界都在本模块内，可随本模块的需求目录闭合（即功能用例）；
> - `后置 → 模块 NN` —— 场景边界跨出本模块、且承接模块尚未落地。该 E2E 场景由 NN 模块的需求
>   负责闭合；本模块的需求**不承接它，也不得长期挂一条无法闭合的验收项**。
>
> 规则：**需求目录只承接标 `本模块` 的场景**，其余以功能用例为限。标 `后置` 的场景必须在 §6
> 追溯矩阵中写出承接方，否则视为缺口。这条维度用于防止「需求按模块切、验收表按用户旅程写」
> 所造成的验收项永久悬空。
...
|--------|--------|--------|---------|-------------|------|---------|---------|---------|
| S-01 | FEAT-01 | P0 | E2E | API → Store → 最终输出 | 本模块 / 后置 → 模块 NN | | | |

**异常场景**

| 场景ID | 功能ID | 测试层级 | 关键真实边界 | 归属 | 触发条件 | 系统行为 | 用户感知 |
|--------|--------|---------|-------------|------|---------|---------|---------|
| E-01 | FEAT-01 | integration | Service → Store | 本模块 / 后置 → 模块 NN | | | |

**边界场景** [按需]

| 场景ID | 测试层级 | 关键真实边界 | 归属 | 字段/条件 | 边界值 | 预期行为 |
|--------|---------|-------------|------|----------|--------|---------|
| B-01 | unit | 校验函数 | 本模块 / 后置 → 模块 NN | | | |
