...
|--------|---------|---------|-------------|---------|------|
| S-01 | adr-wf-001-durable-execution.design.md#2.5.2 功能验收场景 | E2E | Adapter→Engine→durable store（3 候选各自出证据） | TASK-005（证据: TASK-002/003/004） | planned |
| S-02 | 同上 | E2E | 真实 backend + 真实进程 kill | TASK-005（证据: TASK-002/003/004） | planned |
| S-03 | 同上 | integration | ExecutionSnapshot pinned + retention mock | TASK-005（证据: TASK-002/003/004） | planned |
| S-04 | 同上 | integration | Engine step timeout 配置 | TASK-005（证据: TASK-002/003/004） | planned |
| S-05 | 同上 | integration | 真实 backend dedup | TASK-005（证据: TASK-002/003/004） | planned |
| S-06 | 同上 | integration | 2 个 worker 进程 | TASK-005（证据: TASK-002/003/004） | planned |
| E-01 | 同上 | integration | Adapter fail policy + circuit-breaker | TASK-001 | verified |
| E-02 | 同上 | integration | PoC evidence artifact gate | TASK-005 | planned |
| B-01 | 同上 | unit | WorkflowAdapter 不变量 | TASK-001 | verified |
| B-02 | 同上 | integration | Engine tenant scoping | TASK-003（TASK-002/004 引用） | planned |
...
- [ ] `pip install dbos` + 复用 `local_postgres` 建独立 database（如 `fluxion_poc_dbos`）
- [ ] `DBOSWorkflowEngine` 实现 Protocol 5 成员（`@DBOS.workflow()` / `@DBOS.step()` 映射）
- [ ] [S-01][E2E] 真实边界 Adapter→DBOS→Postgres durable store：`execute_workflow` 返回 `run_id` 且 start 同步持久化；断言 durable start P95≤1s（SLO-WF-01）
- [ ] [S-02][E2E] 真实边界 DBOS + 真实进程 kill：worker 中途 kill → `resume(run_id)`；断言从最近 durable step 继续非重启，recovery P95≤60s（SLO-WF-02 / P-CRASH / RISK-WF-01）
- [ ] [P-TIMER][integration] 真实边界 DBOS durable timer：worker 重启后定时器仍触发
- [ ] [P-SIGNAL][integration] 真实边界 DBOS external approval signal：workflow 运行中 `signal(run_id, "approve", payload)` 唤醒等待中的 step 并继续推进（roadmap TASK-0002 项 5）
- [ ] [S-05][integration] 真实边界 DBOS dedup 机制：重试已完成 step；断言 no-op 且不可逆写副作用重复次数=0（P-IDEMP / SLO-WF-03 / NFR-REL-03）
- [ ] [S-03][integration] 真实边界 ExecutionSnapshot pinned + retention mock：长时间 workflow resume；断言使用 pinned WorkflowDefinition version，不 resolve latest（P-PIN / RULE-WF-02）
- [ ] [S-04][integration] 真实边界 DBOS step timeout 配置：单步超时；断言触发定义 fail policy 非无限等待（P-TIMEOUT / RULE-WF-04）
- [ ] [S-06][integration] 真实边界 2 个真实 worker 进程：断言 2nd worker 拉取排队 work（P-SCALE / NFR-SCALE-02）
- [ ] [B-02][integration] 真实边界 DBOS engine tenant scoping：tenant A 的 workflow_run 对 tenant B 查询不可见（NFR-SEC-01）
- [ ] [RULE-backend-database-001][integration] 真实边界 Postgres durable state：commit 后 kill 进程，断言 committed step state 无丢失（RPO=0）
- [ ] verifier: `backend-database#RULE-backend-database-001` — `pytest backend/tests/workflow_poc/test_poc_dbos.py -k "rpo or b02"`（durable state RPO=0 + tenant scope 查询隔离）
- [ ] [P-SCALE+][baseline] 1000 concurrent workflow baseline（roadmap TASK-0002 项 8，首个跑通候选执行，其余候选可选）：1000 并发 workflow 启动/推进，记录吞吐与 P95 写入 evidence
- [ ] [SLO-OBS-01][integration] 真实边界 trace 记录链：断言每口径执行记录 trace_id/run_id 关联完整（≥99%）
- [ ] 产出 evidence JSON artifact（`backend/tests/workflow_poc/evidence/dbos.json`：7 口径 + baseline PASS/FAIL + 计时数据）
...
| 场景ID | 测试层级 | 不得 Mock 的真实边界 | 关键断言 | 测试文件 / 用例 | 执行命令 | 状态 |
|--------|---------|--------------------|---------|----------------|---------|------|
| S-01 | E2E | Adapter→DBOS→Postgres | 返回 `run_id`；start 同步持久化；durable start P95≤1s | backend/tests/workflow_poc/test_poc_dbos.py::test_s01_durable_start | `pytest backend/tests/workflow_poc/test_poc_dbos.py -k s01` | planned |
| S-02 | E2E | DBOS + 真实进程 kill | 从最近 durable step 继续；recovery P95≤60s | test_poc_dbos.py::test_s02_crash_recovery | `pytest backend/tests/workflow_poc/test_poc_dbos.py -k s02` | planned |
| S-03 | integration | ExecutionSnapshot pinned + retention mock | 用 pinned version，不 resolve latest | test_poc_dbos.py::test_s03_pinned_resume | `pytest backend/tests/workflow_poc/test_poc_dbos.py -k s03` | planned |
| S-04 | integration | step timeout 配置 | 触发定义 fail policy，非无限等待 | test_poc_dbos.py::test_s04_step_timeout | `pytest backend/tests/workflow_poc/test_poc_dbos.py -k s04` | planned |
| S-05 | integration | DBOS dedup | 重试已完成 step 为 no-op；副作用重复=0 | test_poc_dbos.py::test_s05_idempotency | `pytest backend/tests/workflow_poc/test_poc_dbos.py -k s05` | planned |
| S-06 | integration | 2 个真实 worker 进程 | 2nd worker 拉取排队 work | test_poc_dbos.py::test_s06_scale_two_workers | `pytest backend/tests/workflow_poc/test_poc_dbos.py -k s06` | planned |
| B-02 | integration | DBOS engine tenant scoping | 跨租户 workflow_run 不可见 | test_poc_dbos.py::test_b02_tenant_isolation | `pytest backend/tests/workflow_poc/test_poc_dbos.py -k b02` | planned |
| RPO | integration | Postgres durable state | commit 后 kill，committed state 无丢失 | test_poc_dbos.py::test_rpo_zero_commit | `pytest backend/tests/workflow_poc/test_poc_dbos.py -k rpo` | planned |
...
| 场景ID | RED | GREEN | 断言位置 | 真实边界证据 | 状态 |
|--------|-----|-------|---------|-------------|------|
| S-01 | FAIL: dbos_app 缺失 | pending | pending | WorkflowAdapter→DBOSWorkflowEngine→DBOS→Postgres（runtime_context 真实快照） | test-written |
| S-02 | 同上 | pending | pending | 真实子进程 SIGKILL + startup recovery | test-written |
| S-03 | 同上 | pending | pending | pinned version 表 + MockRetentionGuard | test-written |
| S-04 | 同上 | pending | pending | step 内 asyncio.timeout 上限 | test-written |
| S-05 | 同上 | pending | pending | DBOS 同 wfid dedup + ON CONFLICT 幂等写 | test-written |
| S-06 | 同上 | pending | pending | 2 真实 worker 进程 + database-backed queue | test-written |
| B-02 | 同上 | pending | pending | poc_runs/poc_records tenant 维度查询 | test-written |
| RPO | 同上 | pending | pending | commit 后 SIGKILL，restart 前后状态断言 | test-written |
...
Restate（Rust 单二进制 durable runtime，invocation journal 模型，Python SDK `restate-sdk`）过 6 口径。license 条款（source-available/FSL 类双许可）确认结果记入 evidence，供评估矩阵"数据与合规"维度打分。

### Checklist

- [ ] 起 Restate runtime（docker `restatedev/restate` 或官方二进制）+ Python SDK 接入
- [ ] `RestateWorkflowEngine` 实现 Protocol 5 成员（virtual object / workflow handler 映射）
- [ ] [S-01][E2E] 真实边界 Adapter→Restate→journal：`execute_workflow` 返回 `run_id` 且 start 同步持久化；断言 durable start P95≤1s（SLO-WF-01）
- [ ] [S-02][E2E] 真实边界 Restate + 真实进程 kill：中途 kill → resume；断言从最近 journal 点继续非重启，recovery P95≤60s（SLO-WF-02 / P-CRASH）
- [ ] [P-TIMER][integration] 真实边界 Restate durable timer：worker 重启后定时器仍触发
- [ ] [P-SIGNAL][integration] 真实边界 Restate external approval signal：运行中 signal 唤醒等待 step 并继续推进（roadmap TASK-0002 项 5）
- [ ] [S-05][integration] 真实边界 Restate dedup：重试已完成 step；断言 no-op 且副作用重复=0（P-IDEMP / SLO-WF-03）
- [ ] [S-03][integration] 真实边界 pinned + retention mock：断言用 pinned version 不 resolve latest（P-PIN / RULE-WF-02）
- [ ] [S-04][integration] 真实边界 step timeout：断言触发定义 fail policy 非无限等待（P-TIMEOUT / RULE-WF-04）
- [ ] [S-06][integration] 真实边界 2 个真实 worker 进程：断言 2nd worker 拉取排队 work（P-SCALE）
- [ ] [B-02 引用][integration] tenant scoping 在 Restate 上复验（负责人 TASK-003）
- [ ] license 条款确认记入 evidence（FSL 类双许可生产使用限制）
- [ ] 产出 evidence JSON（`backend/tests/workflow_poc/evidence/restate.json`）
- [ ] 运行验收命令并填写 Acceptance Evidence
...
| 场景ID | 测试层级 | 不得 Mock 的真实边界 | 关键断言 | 测试文件 / 用例 | 执行命令 | 状态 |
|--------|---------|--------------------|---------|----------------|---------|------|
| S-01 | E2E | Adapter→Restate→journal | 返回 `run_id`；同步持久化；P95≤1s | planned: backend/tests/workflow_poc/test_poc_restate.py::test_s01_durable_start | planned: `pytest backend/tests/workflow_poc/test_poc_restate.py -k s01` | planned |
| S-02 | E2E | Restate + 真实进程 kill | 从最近 journal 点继续；recovery P95≤60s | planned: test_poc_restate.py::test_s02_crash_recovery | planned: `pytest ... -k s02` | planned |
| S-03 | integration | pinned + retention mock | 用 pinned version，不 resolve latest | planned: test_poc_restate.py::test_s03_pinned_resume | planned: `pytest ... -k s03` | planned |
| S-04 | integration | step timeout 配置 | 触发定义 fail policy | planned: test_poc_restate.py::test_s04_step_timeout | planned: `pytest ... -k s04` | planned |
| S-05 | integration | Restate dedup | no-op；副作用重复=0 | planned: test_poc_restate.py::test_s05_idempotency | planned: `pytest ... -k s05` | planned |
| S-06 | integration | 2 个真实 worker 进程 | 2nd worker 拉取排队 work | planned: test_poc_restate.py::test_s06_scale_two_workers | planned: `pytest ... -k s06` | planned |
...
- [ ] [S-01..S-06][汇总] 验证已执行候选 evidence artifact 齐全（`dbos.json`/`restate.json` 各 7 口径 + trace 断言；DBOS 另含 1000-concurrent baseline；Temporal 未触发回补时不要求 `temporal.json`，但矩阵须含其文档打分列与未测理由）+ SLO 数值判定（durable start P95≤1s / recovery P95≤60s / 不可逆副作用重复=0）
- [ ] 19 维度（15 PRD §4.8 + 4 补充：Contract swappability / vendor lock-in / Contract 可替换性 / PoC 失败回退成本）× 3 候选打分回填 design §3.1.2 权重矩阵（self-built 列保持 pending，注明"未测：PRD 默认否决 + 翻盘条件未触发"）
- [ ] [E-02][integration] 真实边界 evidence artifact 存在性 + 时效检查：CI gate 脚本——无有效 PoC evidence 时阻断 WorkflowEngine 生产实现路径（RULE-fluxion-dfx-001：证据须编码阶段自动化产出，非事后补）
- [ ] vendor pick ADR 落 `docs/adr/`（amend ADR-008；解除 `pending-PoC-gate`；记录 self-built fallback 条件 RISK-WF-01：若 3 候选全不达标→升级 self-built PoC 决策，否则降级 FEAT-11 范围）
- [ ] verifier: `fluxion-dfx#RULE-fluxion-dfx-001` — `pytest backend/tests/workflow_poc/test_poc_gate.py`（E-02：evidence artifact 存在性 + 时效 CI gate，编码阶段自动化产出非事后补）
- [ ] 运行验收命令并填写 Acceptance Evidence
...
| 场景ID | 测试层级 | 不得 Mock 的真实边界 | 关键断言 | 测试文件 / 用例 | 执行命令 | 状态 |
|--------|---------|--------------------|---------|----------------|---------|------|
| S-01..S-06 | 汇总判定 | 3 候选 evidence artifact（真实 PoC 产出） | 3 候选 artifact 齐全、每口径 PASS、SLO 数值达标 | planned: backend/tests/workflow_poc/test_evidence_summary.py | planned: `pytest backend/tests/workflow_poc/test_evidence_summary.py` | planned |
| E-02 | integration | evidence artifact gate（真实文件检查） | 无有效 evidence 时 gate 阻断（非零退出） | planned: backend/tests/workflow_poc/test_poc_gate.py | planned: `pytest backend/tests/workflow_poc/test_poc_gate.py` | planned |
