...
**评审边界说明**:
- 本文档是 v2.2 Rolling-wave 的 **Phase 2 Detailed Implementation Plan**（roadmap §11），继承三份规划文档（`fluxion-v2.2-architecture-remediation-roadmap.md` §4、`fluxion-v2.2-enterprise-agent-platform-prd.md` FEAT-06/07/08 + §4.5–4.7、`fluxion-final-source-review-remediation-plan.md` §10）与两份 Phase 0 ADR 设计简报（ADR-MEM-001 / ADR-SNAPSHOT-001）。
...
| **核心目标** | 建立 **ContextResolver 管线**，产出携带 **canonical digest / memory manifest** 的 **ExecutionSnapshot V2**，补齐 **Memory V2**（pgvector/learning control/L2 迁移），落地 **Redis cache** 与 **Multi-Pod 验证**，使相同 `tenant_id + user_id + runtime_profile_id` 跨 Pod 解析出一致的 Snapshot（架构规则 28）。 |
...
| FEAT-P2-03 | Personal Memory 迁出 runtime 域 | `runtime/personal_memory.py` → `users/` 域（User/Memory State Plane） | P0 | US-02 / remediation-plan P-04 |
...
| FEAT-P2-10 | Redis tenant cache adapter | tenant-scoped key、TTL、invalidation、degraded fallback、clear-all correctness；供 ContextResolver 做 L2 | P1 | roadmap Gate 1C / Phase 2 依赖 |
| FEAT-P2-11 | Multi-Pod 验证 | 3-pod routing E2E、local cache clear、kill -9、Redis restart/degrade（真实 k8s + PG + Redis） | P1 | US-09 / roadmap R214-R217 / Phase 2 Gate |
...
| **前置假设** | ADR-MEM-001 / ADR-SNAPSHOT-001 / ADR-EXT-001 / ADR-005 accepted；AgentDefinition/UserProfile/CapabilityGrant 已落地（phase1）；M201 + M209-M216 已落地；本地 k8s + docker PostgreSQL（`mmuser/mmuser@localhost:5432`，fluxion_test 库）+ Redis（`localhost:6379` 无密码）可用（复用记忆 `local-pg-test-env`）。 |
...
| ID | 类型 | 描述 | 验证场景 |
|----|------|------|---------|
| RULE-P2-01 | 系统约束 | 相同 `tenant_id + user_id + runtime_profile_id` 在不同 Pod 解析出相同 `snapshot_digest`（100% 一致，架构规则 28） | S-01 |
...
|--------|--------|--------|---------|-------------|---------|---------|---------|
| S-01 | FEAT-P2-01/02 | P0 | E2E | Product API → Service → ContextResolver → Registry → Secret → Memory | 已发布 agent_definition + user_profile + binding + personal memory 条目 | 两个独立 Application 实例（共享同一真实 PG + Redis）各 resolve 同一 runtime_profile | 两份 `snapshot_digest` 完全相等；含 `user_profile_version`/`memory_manifest`/`credential_versions` |
...
| S-06 | FEAT-P2-11 | P0 | E2E | k8s 3 Pod → PG → Redis | 3 副本运行 | 分别服务请求后 kill -9 一个 Pod，新请求打到存活 Pod | digest 一致；新请求正常，RPO=0 |
...
| 决策点 | 选择 | 被否决项 | 理由 | 可逆性 |
|--------|------|---------|------|--------|
| ContextResolver 归属 | **`services/context_resolver.py` 应用服务** | `users/context.py`（域内编排跨域职责）；`runtime/context.py`（违背 P-04） | ContextResolver 是跨 Agent/User/Registry/Secret 四域的 use case 编排；放 services 符合依赖方向 `services -> domain contracts`；Personal Memory 数据/查询迁 `users/`（User/Memory State Plane），满足 P-04「移出 Runtime Domain」 | 中（纯内部包移动 + DI，API 不变） |
...
| digest 算法 | **canonical JSON（递归排序键 + 忽略 None/空值）+ sha256** | 签名/HMAC（跨 Pod 一致性无需密钥）；UUID 随机摘要（不可验证） | 确定性、可复现、跨 Pod 可断言相等；只含「版本事实」，排除 `created_at`/`execution_id`/`trace_id` 等运行时字段 | 易 |
...
```mermaid
graph TB
    subgraph "users/ 域 (User/Memory State Plane)"
        U1["models.py (Profile/Preference)"]
        U2["service.py (UserDomainService)"]
        U3["personal_memory.py (迁入: Store/Learner/Retriever)"]
    end
...
| 函数签名 | 入参 | 返回 | 错误处理 |
|---------|------|------|---------|
| `ContextResolver.resolve(request: RequestContext, selector: ResolverSelector) -> ResolveResult` | 请求上下文 + 选择器（profile/agent/memory budget） | `ResolveResult`（`ExecutionSnapshot` + `UserContext` + `resolution_trace` + `budget_used`） | `ContextResolutionError`（带 slug + 整码）；fail-closed |
...
|--------|---------|-------|------------------------------|
| NFR-PERF-01 | ContextResolver 十段解析 | P95 ≤ 300ms | **L1 内存 + Redis L2 缓存** profile/capability/definition 解析结果，只对变更版本回源；放弃「每 execution 全量回源」（每段一次 Store 读，最坏 10 次 DB IO） |
...
|---------|--------|--------|-----------|---------|------|
| US-09 | FEAT-P2-01 | `ContextResolver.resolve` | S-01/S-02/E-01/E-04 | E2E/integration | 待实现 |
| US-09 | FEAT-P2-02 | `canonical_digest` | S-01/B-02/B-03 | E2E/unit | 待实现 |
| US-02 | FEAT-P2-03 | 包迁移 | S-03 | integration | 待实现 |
...
| — | FEAT-P2-07 | 迁移脚本 | M202 | manual（数据迁移 dry-run） | 待实现 |
| US-03 | FEAT-P2-08 | `PersonalMemoryStore.reindex` | manual（外部 UI 依赖，后端契约已覆盖） | integration | 待实现 |
| US-03 | FEAT-P2-09 | `UserPreference` + Learner gate | S-04 | E2E | 待实现 |
| US-02 | FEAT-P2-10 | `TenantRedisCache` | S-05/E-05 | E2E/integration | 待实现 |
| US-09 | FEAT-P2-11 | k8s 验证 | S-06 | E2E | 待实现 |

> RULE-P2-01~07 分别映射到 S-01（digest 一致）、S-02（P95）、S-03（架构）、E-02（secret）、S-04（停学）、S-05（Redis degrade）、S-06（kill -9 RPO=0）。矩阵闭合无断点。
...
| User/Memory State Plane | P-04 提出的个人记忆归属域（本 phase 迁入 `users/`） |
