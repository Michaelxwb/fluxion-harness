...
### Checklist
- [ ] 先写测试并记录 RED：解除不存在关系应 200（当前 404）；列表无分页封套；sort_order 恢复不更新
- [ ] [S-02][E2E] 绑定 Skill 立即写入（真实 HTTP+PG），后续 resolve 可见，无全局保存
- [ ] [E-03][integration] 绑定不存在/软删 Skill → COMMON_NOT_FOUND；禁用 Skill 绑定成功且 resolve 过滤（不进 Prompt/Catalog）
- [ ] 解除幂等：不存在/已删除关系返回成功 `{agent_id,skill_id,is_deleted:true}`
- [ ] 列表分页 `{items,page,page_size,total}`；字段 current_artifact_version/create_time；排序 sort_order ASC, create_time ASC
- [ ] 运行 harness-rel#RULE-rel-001 verifier：仅单关系 POST/DELETE，无全量 PUT
- [ ] 同步修正 tests/console_skill/test_bindings_api.py 固化的非幂等断言
- [ ] 运行验收命令并填写 Acceptance Evidence
...
- **Spec-Refs**: -（授权规则责任任务为 TASK-004，此处遵守其公式与幂等约定）
...
### Checklist
- [ ] 先写测试并记录 RED：/agents/{id}/users 端点不存在
- [ ] [S-04][E2E] 授权后 resolve 可用；撤销=软删除，后续 resolve 拒绝，历史 Snapshot 不漂移
- [ ] [E-04][integration] 重复授权幂等返回既有 Grant；软删行恢复更新 granted_by/granted_at；无重复有效行
- [ ] 列表分页封套 + keyword（user_code/display_name）；JOIN platform_user
- [ ] Agent 不存在 → AGENT_NOT_FOUND（非 COMMON_NOT_FOUND）
- [ ] Builder 角色可调用（不挂 admin-only router）
- [ ] 运行验收命令并填写 Acceptance Evidence
...
### Checklist
- [ ] 先写测试并记录 RED：/api/v1/audits 端点不存在
- [ ] [B-04][integration] 统一封套/分页边界/按 resource_id 过滤/时间倒序（真实 HTTP + 真实 PostgreSQL）
- [ ] 响应不含 Secret/明文字段（复用 sanitize）
- [ ] 运行 harness-api#RULE-api-001 verifier：封套字段、分页约束、错误码映射
- [ ] 运行验收命令并填写 Acceptance Evidence
