...
|---|---|---|---|---|
| FEAT-FE-01 | 后台任务列表/详情 | 任务状态、进度、时间、投递状态、Timeline、子任务。 | P0 | 需求描述 |
| FEAT-FE-02 | 任务取消 | 仅非终态显示取消操作。 | P0 | 需求描述 |
| FEAT-FE-03 | 定时任务列表/详情 | 调度规则、下次/最近触发、按 schedule_id 查询历史 Task。 | P0 | 需求描述 |
...
| 页面 | 路由 | 布局 | 说明 |
|---|---|---|---|
| 后台任务 | `/tasks` | ConsoleShell | TaskExecution 列表 + 详情 SideSheet |
| 定时任务 | `/schedules` | ConsoleShell | TaskSchedule 列表 + 详情 SideSheet |
...
|---|---|---|---|---|
| CMP-01 | `TaskPage` | 容器 | 模块内 | 任务过滤/分页/详情/取消 |
| CMP-02 | `TaskTimeline` | 展示 | 模块内 | TaskEvent Timeline |
| CMP-03 | `SchedulePage` | 容器 | 模块内 | Schedule 查询/动作 |
...
| `mss-platform#RULE-WORKER-001` | required | PG 是 Task/Schedule/lease 权威源；Redis 仅 hint；TaskType V1=SKILL/BATCH。 | §3.3/§3.5 | S-FE-01 + verifier | applied |
