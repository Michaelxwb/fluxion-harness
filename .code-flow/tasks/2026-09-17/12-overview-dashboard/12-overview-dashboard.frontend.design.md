...
| 模块名称 | 概览与运营入口 |
| 需求类型 | 页面/组件/交互实现 |
| 核心目标 | 保留交互稿概览的信息层级，作为业务运营入口而不是运维监控。 |

### 2.2 功能方案

| 功能ID | 功能名称 | 功能描述 | 优先级 | 来源 |
|---|---|---|---|---|
| FEAT-FE-01 | KPI 卡片 | 启用 Agent/Skill、后台执行中、启用定时任务。 | P0 | 需求描述 |
| FEAT-FE-02 | 运行关系说明 | IM→Agent→Runtime→ExecutionRouter→Worker/DB/Gateway。 | P0 | 需求描述 |
...
| 类别 | 内容 |
|---|---|
| In Scope | 首页 KPI、架构关系静态说明、最近 Task/Schedule、导航链接。 |
...
|---|---|---|---|---|---|
| S-FE-01 | FEAT-FE-01 | E2E | Browser→overview API | 打开首页 | 4 KPI + 最近任务/调度一次加载 |
| S-FE-02 | FEAT-FE-03 | E2E | Browser Router | 点击查看全部 | 进入 tasks/schedules 且菜单选中正确 |
