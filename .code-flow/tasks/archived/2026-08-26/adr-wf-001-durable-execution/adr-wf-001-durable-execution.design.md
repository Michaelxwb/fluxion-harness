...
> 打分基于 PoC 实测证据（2026-08-27 回填）。权重锁定为评分依据；**15 PRD §4.8 维度 + 4 补充维度（ADR-008 派生：Contract swappability / vendor lock-in / Contract 可替换性 / PoC 失败回退成本）= 19 项**归入 5 个宏维度（补充维度已在“生态与适配”“风险/可逆性”行标注）。
>
> **实测证据基准**：DBOS（`evidence/dbos.json` all_criteria_passed=True，11/11 场景 + baseline 1000 并发 + SLO 全达标）；Restate（`evidence/restate.json` 5 口径通过 + **S-02/P-TIMER/S-03/RPO 单节点崩溃恢复不可用 + S-06 单节点 2 deployment 不分摊**，用户决策记 capability boundary）；**Temporal 不对比**（2026-08-27 用户标记：组件太复杂，独立 server 集群 + DB 运维成本与收益不匹配，未实测、不参与候选对比，列仅按文档作参考并注明"未实测/不对比"）；Self-built 未测（PRD 默认否决，翻盘条件未触发）。

| 宏维度 | 权重 | 含义（PRD §4.8 维度） | Temporal* | DBOS | Restate | Self-built |
|--------|------|---------------------|-----------|------|---------|------------|
| 功能完备性 | 30% | durable timer/wait/signal + retry/idempotency + replay/recovery model | 9 | **9** | 6 | pending |
| 生态与适配 | 20% | Python SDK/生态 + dynamic DSL 适配 + ADR-008 Contract swappability | 9 | **8** | 7 | pending |
| 运维 | 20% | self-host + horizontal scaling + operational complexity + local dev complexity + observability | 3 | **9** | 4 | pending |
| 数据与合规 | 20% | data ownership（RPO=0/tenant）+ license + upgrade/versioning + workflow retention + enterprise maturity | 8 | **9** | 5 | pending |
| 风险/可逆性 | 10% | vendor lock-in + Contract 可替换性 + PoC 失败回退成本 | 7 | **8** | 5 | pending |
| **最终得分** | **100%** | | **7.4** | **8.7** | **5.5** | **—** |

> \* Temporal **不对比**（2026-08-27 用户标记：组件太复杂），列仅文档参考不参与对比；**实测对比 DBOS 8.7 vs Restate 5.5，DBOS 最高 → vendor pick = DBOS**（ADR-013 已解除 `pending-PoC-gate`）。打分理由：DBOS 全口径实测通过 + 零新增服务（复用 Postgres）+ MIT 开源 + 进程级恢复自动生效；Restate 崩溃恢复与 scale 单节点硬伤（运维/故障恢复强扣分）。
