...
| Secret | 密钥明文存于各 Owner 表（模型 `api_key`、Bot `secret`、MCP/平台 `auth_secret`、用户/共享 `credential_json`），跨表只按主键引用；不使用 SecretRef/SecretProvider；密钥不进入本模块 task 表、Snapshot、事件、日志、审计、LLM、IM 与公开 API |
...
| RULE-04 | 系统约束 | 密钥明文只存于各 Owner 表并以主键引用（无 SecretRef/SecretProvider）；本模块 task schema 各表、Snapshot、事件、日志、审计、LLM、IM 与公开 API 均不得携带密钥，对外只回 `*_configured`。 | S-01 |
...
| RULE-11 | 业务规则 | Schedule 错过触发 V1 仅 SKIP（不补发），无 `misfire_policy` 字段/API/metric label；跳过记审计并累加 `scheduled_misfire_total`；ONCE 触发成功后 `status=COMPLETED`、写 `completed_at`；ONCE 错过触发时间进入终态 `status=MISSED`（`completed_at=NULL`、`next_fire_at=NULL`），不可恢复、不补发。 | E-04 |
| RULE-12 | 业务规则 | fan-out Child 幂等键 `parent:{parent_id}:{item_key}`，唯一约束 `(parent_id, item_key)`；fan-in 原子检查兄弟终态并 CAS 完成/唤醒 Parent。 | S-04 |
| RULE-13 | 业务规则 | Final Delivery at-least-once：`delivery_key=task:{id}:final`；Gateway 以 Redis `delivery:dedupe:{delivery_key}` SET NX EX 7d 去重，但 SET NX 成功只代表「已占位」而非「已送达」，发送失败或占位后崩溃必须允许重试且不得据此置 SENT；Redis 正常时重放保证不重复发送，Redis 不可用或外部发送结果不确定时只承诺 at-least-once（可能重复），不宣称外部渠道 exactly-once；Worker 指数退避最多 5 次。 | S-03 / E-05 |
...
| Final Delivery | `delivery_key` + Gateway Redis SET NX EX 7d | 终态直接发送无幂等 | at-least-once 投递 + 去重；占位≠送达，Redis 不可用时降级为可能重复 |
...
- **占位语义**：`SET NX` 成功只表示该 `delivery_key` 已被占位，**不等于已送达**；渠道发送失败、或占位后进程崩溃重启时必须释放占位并允许重试，禁止仅凭占位把 `delivery_status` 置为 `SENT`；
- **故障边界**：Redis 正常时同一 `delivery_key` 的重放保证不重复发送；Redis 不可用、或外部发送结果不确定（超时/连接中断）时只承诺 **at-least-once**，允许重复投递，**不宣称外部渠道 exactly-once**；
...
- **安全边界**：只保存 `actor_user_id` 与凭据归属主键，不保存密钥；每次触发经既有凭据边界重新校验 Agent 授权与凭据状态。
...
| `status` | varchar(16) | NOT NULL DEFAULT 'ACTIVE' | ACTIVE/PAUSED/COMPLETED/MISSED；删除只用 is_deleted |
...
| `update_time` | timestamptz | NOT NULL DEFAULT now() | 更新时间 |
| `tenant_id` | varchar(64) | NOT NULL | 隔离键 |
| `task_id` | uuid | NOT NULL FK -> task_execution.id | TaskExecution |
...
ScheduleStatus:
ACTIVE / PAUSED / COMPLETED / MISSED
（删除由 is_deleted=true 表达；ONCE 成功创建唯一 TaskExecution 后进入 COMPLETED；
  ONCE 错过触发时间进入终态 MISSED，completed_at/next_fire_at 均为 NULL，不可恢复、不补发）
...
- Redis 丢失后允许 cache miss，但不得丢失 Task/Schedule/授权关系；`delivery:dedupe` 不可用时投递降级 at-least-once（可能重复，不承诺 exactly-once），且 `SET NX` 占位不代表送达——发送失败或占位后崩溃必须允许重试；
...
erDiagram
 ...
 TASK_SUBMISSION ||--o| TASK_EXECUTION : "首次提交产生"
```

数据库规则：所有产品表统一 `id/is_deleted/create_time/update_time`；同 Owner Schema 物理 FK，跨 Owner Schema 逻辑 UUID。

#### 3.3.8 `task.task_submission`

**表说明**

- **用途**：创建类 POST（`POST /internal/tasks`、`POST /internal/schedules`）的提交幂等记录，保存首次提交的请求指纹与首次响应，供同 key 重放。与模块 08 的 `runtime.run_submission` 同构。
- **主要写入方**：Agent Worker Task/Schedule API 的提交入口。
- **主要读取方**：同一提交入口的重放路径。
- **生命周期/边界**：与业务行同一事务写入；只保存规范化指纹与首次响应快照，不保存密钥。
...
| `update_time` | timestamptz | NOT NULL DEFAULT now() | 更新时间 |
| `tenant_id` | varchar(64) | NOT NULL | 隔离键 |
| `idempotency_key` | varchar(128) | NOT NULL | 取自 `Idempotency-Key` Header，或 body `idempotency_key` |
| `endpoint` | varchar(32) | NOT NULL | 端点 slug：`create-task` / `create-schedule` |
| `request_fingerprint` | varchar(128) | NOT NULL | 规范化 JSON 指纹，`sha256:` 前缀 |
| `actor_user_id` | uuid | NOT NULL | 提交主体 |
| `task_id` | uuid | FK -> task_execution.id | 首次提交产生的 Task |
| `schedule_id` | uuid | FK -> task_schedule.id | 首次提交产生的 Schedule |
| `response_json` | jsonb | NOT NULL | 首次持久化结果快照，用于重放 |

**索引/约束**：

- `uq_task_submission_tenant_key_endpoint`：`UNIQUE (tenant_id, idempotency_key, endpoint) WHERE is_deleted=false`
- `INDEX (task_id, create_time)`、`INDEX (schedule_id, create_time)`
- 指纹口径与 `runtime.run_submission` 一致：规范化 JSON（`sort_keys=True`、`separators=(",", ":")`、`ensure_ascii=False`）的 SHA256，前缀 `sha256:`；载荷含 endpoint、tenant/actor、资源 ID 与关键参数。
- 同一 `(tenant_id, idempotency_key, endpoint)` 的并发插入由 partial unique 兜底：落败者回读首次提交结果并按同 key 同指纹语义返回。
...
- Header：`Idempotency-Key`(string,选)；与 body `idempotency_key` 同时出现时必须一致，否则返回 `IDEMPOTENCY_MISMATCH`；同 key 同指纹重放首次响应，不重复创建 Task。
...
- 错误码：`COMMON_VALIDATION_ERROR / COMMON_NOT_FOUND / COMMON_CONFLICT / IDEMPOTENCY_MISMATCH / COMMON_INTERNAL_ERROR`。
- 处理：同一事务写入 `task_execution`（`task_type=SKILL`、`deadline_at=now()+interval '24 hours'`、`delivery_key=task:{id}:final`）、`task_event(CREATED)` 与 `task_submission`（`endpoint='create-task'`、规范化指纹、首次响应快照）；`delivery_route` 按 `route_hash=sha256(规范化路由元组)` 复用或新建；同 `(tenant_id, idempotency_key, endpoint)` 同指纹重复请求重放首次响应，不同指纹返回 `IDEMPOTENCY_MISMATCH`；提交后发布 `task:wakeup` hint（失败不影响事务）。
- 对应章节：`docs/07 §5.1`。
...
- Header：`Idempotency-Key`(string,选)；与 body `idempotency_key` 同时出现时必须一致，否则返回 `IDEMPOTENCY_MISMATCH`；同 key 同指纹重放首次响应，不重复创建 Schedule。
...
- 错误码：`COMMON_VALIDATION_ERROR / COMMON_NOT_FOUND / IDEMPOTENCY_MISMATCH / COMMON_INTERNAL_ERROR`。
- 处理：校验 CRON 必有 `cron_expr`、ONCE 必有 `run_at`；按 `timezone` 计算 `next_fire_at`；`delivery_route` 按 `route_hash` 复用或新建；同一事务写入 `task_schedule` 与 `task_submission`（`endpoint='create-schedule'`、规范化指纹、首次响应快照）；同 `(tenant_id, idempotency_key, endpoint)` 同指纹重复请求重放首次响应，不同指纹返回 `IDEMPOTENCY_MISMATCH`；不保存 Artifact checksum，每次触发重新解析。
- 对应章节：`docs/07 §5.2`。
...
- 请求（query）：`schedule_id`(uuid,选)、`agent_id`(uuid,选)、`actor_user_id`(uuid,选)、`status`(string,选)、`trigger_type`(string,选)、`start_time`(ISO8601,选)、`end_time`(ISO8601,选)、`deadline_from`(ISO8601,选)、`deadline_to`(ISO8601,选)、`page`(int,选,默认 1)、`page_size`(int,选,默认 20,最大 100)。
- `data`：`{ "items": [Task 摘要], "page": 1, "page_size": 20, "total": 123 }`。
- 错误码：`COMMON_VALIDATION_ERROR / COMMON_INTERNAL_ERROR`。
- 处理：按 `tenant_id` 隔离 + 条件 AND；`start_time`/`end_time` 作用于 `create_time`，`deadline_from`/`deadline_to` 作用于 `deadline_at`（均按 UTC 比较、起止含端点）；排序 `create_time DESC`；单条 SQL 分页，禁止 N+1；只读无事务副作用。
- 对应章节：`docs/07 §5.3`。
...
> 注：本模块的 `REVISION_CONFLICT` 指**乐观并发前置不满足**——即 CAS 的目标状态已被他人推进（如 Task 已终态、Schedule 已 `COMPLETED`/`MISSED`/已删除），与本表的更新时间戳无关；该码的语义已从「模型 revision CAS」推广为「revision 或状态前置不满足」。
...
- 处理：校验 `actor_user_id` 为 Schedule owner（Agent 调用）或 Admin/Builder（Console）；仅影响未来触发，`revision=revision+1` 并重算 `next_fire_at`；`COMPLETED`/`MISSED`/已删除不可更新（`REVISION_CONFLICT`）；不修改已创建 Task 的 Snapshot。
- 对应章节：`docs/07 §5.3`。
...
- 请求（query）：`schedule_id/agent_id/actor_user_id/status/trigger_type/start_time/end_time/deadline_from/deadline_to/page/page_size`（同 API-03）。
...
- 错误码：`COMMON_NOT_FOUND / REVISION_CONFLICT / UNAUTHORIZED / FORBIDDEN / COMMON_INTERNAL_ERROR`。
- 处理：仅 `ACTIVE` 可暂停，CAS `status=PAUSED` 并保留 `next_fire_at`；已 `PAUSED` 幂等返回；`COMPLETED`/`MISSED`/已删除返回 `REVISION_CONFLICT`；失败不做乐观更新。
- 对应章节：`docs/07 §10.10`。
...
- `data`：`{ "schedule_id": "uuid", "status": "ACTIVE", "next_fire_at": "YYYY-MM-DD HH:mm:ss" }`。
- 错误码：`COMMON_NOT_FOUND / REVISION_CONFLICT / UNAUTHORIZED / FORBIDDEN / COMMON_INTERNAL_ERROR`。
- 处理：仅 `PAUSED` 可恢复，CAS `status=ACTIVE` 并按当前时间重算 `next_fire_at`（错过的触发按 SKIP 不补发）；`COMPLETED`/`MISSED`/已删除返回 `REVISION_CONFLICT`（`MISSED` 为终态，不提供恢复）。
- 对应章节：`docs/07 §10.10`。
...
GET  /internal/admin/schedules/{schedule_id}
PUT  /internal/admin/schedules/{schedule_id}/pause
PUT  /internal/admin/schedules/{schedule_id}/resume
DELETE /internal/admin/schedules/{schedule_id}
```

- 调用方：Console Platform（`docs/07 §9.4`）；浏览器不直接访问。
- 请求：查询参数与对应 Internal API（API-03/04/05/06/08）一致；暂停/恢复无 body。
...
| RISK-02 | 重复投递 | 用户收到重复消息 | `delivery_key` + Gateway Redis `delivery:dedupe:{delivery_key}` SET NX EX 7d；占位≠送达，Redis 故障或外部结果不确定时降级 at-least-once | E-05 |
...
- 密钥明文只存于各 Owner 表（`model_definition.api_key` / `bot_account.secret` / `mcp_server.auth_secret` / 平台与用户凭据表），本模块 task schema 各表、Snapshot、事件、日志、审计、LLM、IM 与公开 API 均不携带密钥；
...
数据迁移遵循 `docs/02 §11` 的 expand → deploy → contract 原则；`task` schema 既有表由 `migrations/versions/0002_initial_schema.py` 建立，本模块新增的 `task.task_submission` 以独立增量 migration 添加，不修改 0002；模块代码不得自行建表或执行 destructive migration。
...
|---|---|---|---|---|---|
| 需求描述 | FEAT-01 | API-01, API-03, API-04, API-05, API-09, API-10, API-11, API-17 | S-01, S-04, E-01, E-03, E-06, E-07 | E2E / integration | 待实现 |
| 需求描述 | FEAT-02 | API-03, API-04, API-05 | E-01, E-06 | integration | 待实现 |
| 需求描述 | FEAT-03 | API-02, API-06, API-07, API-08, API-12, API-13, API-14, API-15, API-16 | S-02, E-02, E-04, E-07 | E2E | 待实现 |
...
|---|---|---|---|---|---|
| `harness-arch#RULE-arch-001` | required | 固定 4 个部署单元；Runtime/Worker 无状态横向扩展，Task/Schedule/lease 与 Pod 解耦。 | §3.2.2 / §3.3.3 / §4 | S-01, E-01 + verifier | applied |
| `harness-api#RULE-api-001` | required | JSON REST 统一封套与分页；业务只抛 code，msg/http_status 配置映射；错误码仅用已登记代码。 | §3.4 API-01 ~ API-17 | S-01 + verifier | applied |
| `harness-api#RULE-api-002` | required | 创建类 POST 支持 `Idempotency-Key` Header：独立提交幂等表 partial unique `(tenant_id, idempotency_key, endpoint)` 记录首次提交；请求指纹 = 规范化 JSON（`sort_keys` + 紧凑分隔符）的 SHA256，含 endpoint/tenant/actor/资源与关键参数；同 key 同指纹重放首次持久化结果，同 key 不同指纹返回 `IDEMPOTENCY_MISMATCH`；body `idempotency_key` 保持兼容且两者同时出现时必须一致；并发插入由 partial unique 兜底，落败者读取首次结果。 | §3.3.8 / §3.4 API-01, API-02 | E-07 + verifier | applied |
| `harness-data#RULE-data-001` | required | 产品表统一标准列；软删除 partial unique；timestamptz；同 Owner Schema 物理 FK。 | §3.3.1 ~ §3.3.4 | E-01 + verifier | applied |
| `harness-secret#RULE-secret-001` | required | 密钥明文只存于各 Owner 表并按主键引用（无 SecretRef/SecretProvider）；本模块 task schema 各表、Snapshot、事件、日志、审计、LLM、IM 与公开 API 均不携带密钥，对外只回 `*_configured`；delivery_route 只存 `bot_id` 不含 Bot Secret。 | §3.3.3 / §3.5 | S-01 + verifier | applied |
| `harness-snapshot#RULE-snapshot-001` | required | 新 Task 冻结 execution snapshot（含 prompt_template_version、mcp catalog revision/hash）；配置/授权变更只影响后续 Task；终态 CAS。 | §3.3.3 / §3.2.2 | S-02 + verifier | applied |
| `harness-worker#RULE-worker-001` | required | PG 为 Task/Schedule/lease 权威源；Redis 仅 hint；TaskType 仅 SKILL/BATCH；claim `FOR UPDATE SKIP LOCKED`；WAITING 释放 lease；deadline sweep。 | §3.1 / §3.2.1 / §3.2.2 / §3.3.3 | S-01, E-01, E-03 + verifier | applied |
| `harness-skill#RULE-skill-001` | required | `skill_artifact_id` 冻结且不可变；Worker 消费端经 SkillArtifactCache 以 emptyDir 本地缓存、校验 checksum 后原子切换 READY，同一 checksum 并发首次加载 singleflight；禁止直接从 NFS 执行 Python Skill；无 DB 记录的孤儿 Artifact 由 `cleanup-skill-orphans` 按宽限期（默认 1h）扫描清理，保护进行中的导入事务。 | §3.3.3 / §3.2.3 | S-01 + verifier | applied |
| `harness-im#RULE-im-001` | required | delivery_route 只存 bot_id/外部接收方；bot_id 唯一归属 Agent；不绑定 Runtime/Worker Pod。 | §3.3.2 / §3.2.5 | S-03 + verifier | applied |
| `harness-test#RULE-test-001` | required | 关键流程 E2E，明确不得 mock 的真实边界（PG/Redis/HTTP/IM Gateway）。 | §2.5.2 / §3.5 | S-01, S-03, E-01, E-03, E-05 + verifier | applied |
