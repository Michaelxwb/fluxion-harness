...
class ContextResolver:
    ...
    async def resolve(
        ...
        *,
        ...
        # 1. identity：tenant/user 非空即视为已解析（上游 middleware/调用方保证）
        ...
        _stage("identity", selector.user_id, started)

        # 2. user：PlatformUser 存在性（users 表可空——非阻塞，记录版本占位）
        started = time.perf_counter()
        user_version = await self._latest_user_profile_version(selector.tenant_id, selector.user_id)
        _stage("user", user_version, started)
        ...
        # 5. profile：User Profile 版本（可选；pin 校验 fail-closed）
        started = time.perf_counter()
        ...
        _stage("profile", user_profile_version, started)

        # 6. memory：PersonalMemoryRetriever recall → manifest（失败降级空 manifest）
        started = time.perf_counter()
        manifest = await self._memory_manifest(selector.tenant_id, selector.user_id, memory_query, memory_budget)
        _stage("memory", manifest.content_hash or None, started)

        # 7. capability：Agent capabilities（typed 三元组透传）
        started = time.perf_counter()
        capabilities = [
        ...
        started = time.perf_counter()
        credential_versions = await self._credential_versions(selector.tenant_id, selector.user_id)
    ...
    async def _memory_manifest(
        ...
        """Memory 段：recall → manifest；检索失败降级空 manifest + truncated=true。"""
        ...
        except Exception as exc:  # noqa: BLE001 - memory 段降级空 manifest 不阻塞
            del exc
        ...
        refs = [
            MemoryEntryRef(
                ...
                content_hash=_short_hash(row["content"]),
                priority=index,
            )
        ...
        return MemoryManifest(
            entry_refs=refs,
            content_hash=canonical_digest(refs and refs[0] or MemoryEntryRef(entry_id="0", memory_type="semantic", content_hash=""))[:16],
            truncated=truncated,
        )
