"""DBOS PoC 验收测试（ADR-WF-001 TASK-003）。

覆盖 Acceptance Contract 8 场景（S-01/S-02/S-03/S-04/S-05/S-06/B-02/RPO）
+ 7 口径中的 P-TIMER/P-SIGNAL 专项 + SLO-OBS-01 trace 关联断言；
结果写入 `evidence/dbos.json`（TASK-005 矩阵回填数据源）。

真实边界：
- S-01 走 WorkflowAdapter → DBOSWorkflowEngine → DBOS → PostgreSQL（E2E）；
- S-02/S-03/RPO/P-TIMER 用真实子进程 SIGKILL（非 mock 故障注入）；
- S-06 用 2 个真实 worker 进程 + database-backed queue；
- DB 客户端 API 统一 `asyncio.to_thread(同步 API)`（DBOS *_async 绑定首个
  event loop，pytest 每测试新 loop 会挂——该限制记入 evidence）。
"""

from __future__ import annotations

import asyncio
import json
import time
import uuid

import pytest
from tests.runtime_helpers import runtime_context

from fluxion.runtime.workflow import (
    FailPolicy,
    ResilientWorkflowEngine,
    WorkflowAdapter,
    WorkflowStartRequest,
)
from tests.workflow_poc.dbos_app import (
    DBOSWorkflowEngine,
    attach_correlator,
    launch_dbos,
    poc_durable_workflow,
    purge_stale_enqueued,
    reset_business_tables,
    resolve_db_url,
    run_id_for,
)
from tests.workflow_poc.dbos_testing import (
    WorkerProcess,
    record_evidence,
    wait_for_condition,
    write_dbos_evidence,
)
from tests.workflow_poc.poc_workflow import (
    MockRetentionGuard,
    RetentionBlockedError,
    TraceCorrelator,
)


def _eid(prefix: str) -> str:
    return f"{prefix}-{uuid.uuid4().hex[:8]}"


def _start_request(tenant_id: str, eid: str, **arguments: object) -> WorkflowStartRequest:
    return WorkflowStartRequest(
        workflow_id="poc-durable",
        tenant_id=tenant_id,
        user_id="user-poc",
        execution_id=eid,
        trace_id=f"trace-{eid}",
        arguments={"execution_id": eid, **arguments},
    )


@pytest.fixture(scope="module")
def engine() -> DBOSWorkflowEngine:
    # listen=[]：测试驱动进程不监听用户队列，S-06 的 poc_queue 只允许 2 个真实
    # worker 消费（否则 main 的 queue_thread 会抢先批量认领，executor_id 全是
    # 'local'，测不出 2nd worker 生效）。
    instance = DBOSWorkflowEngine(listen=[])
    yield instance
    write_dbos_evidence()


@pytest.fixture(autouse=True)
def _clean_tables() -> None:
    reset_business_tables(resolve_db_url())
    yield


@pytest.fixture(scope="module", autouse=True)
def trace_events() -> TraceCorrelator:
    correlator = TraceCorrelator()
    attach_correlator(correlator)
    yield correlator


async def test_s01_durable_start(engine: DBOSWorkflowEngine) -> None:
    """S-01[E2E]: Adapter→DBOS→Postgres；start 同步持久化且单次延迟 ≤1s（SLO-WF-01）。"""
    context, _runtime = await runtime_context()
    resilient = ResilientWorkflowEngine(
        delegate=engine,
        policy=FailPolicy(
            timeout_seconds=10.0, max_retries=0, breaker_threshold=3, breaker_cooldown_seconds=60.0
        ),
    )
    adapter = WorkflowAdapter(workflow_id="poc-durable", engine=resilient)

    eid = _eid("s01")
    started = time.monotonic()
    tool_result = await adapter.execute(context, {"execution_id": eid, "approval_timeout_seconds": 5.0})
    start_ms = (time.monotonic() - started) * 1000
    run_id = run_id_for("poc-durable", eid)

    assert tool_result.run_id == run_id  # Adapter 返回 durable run_id
    status = await engine.get_status(run_id)  # start 返回即可查 → 同步持久化
    assert status.status in {"PENDING", "RUNNING", "ENQUEUED", "SUCCESS"}

    await engine.signal(run_id, "approve", {"approved": True, "reviewer": "r1"})
    result = await engine.await_result(run_id, timeout=30.0)
    assert result["approval"] == {"approved": True, "reviewer": "r1"}
    assert result["pinned_version"] == "v1"
    assert (await engine.get_status(run_id)).status == "SUCCESS"

    records = dict.fromkeys(engine.list_records("tenant-a"))  # runtime_context tenant=tenant-a
    assert (run_id, "report") in records
    assert (run_id, "http") in records
    executions = engine.step_executions(run_id)
    assert executions["write_report_record"] == 1
    assert executions["notify_http_endpoint"] == 1
    assert start_ms <= 1000.0  # SLO-WF-01：durable start ≤1s（单样本；P95 口径见 evidence）

    record_evidence(
        "S-01",
        passed=True,
        detail="E2E Adapter→DBOS→Postgres；start 返回即同步持久化（status 可查）",
        metrics={"start_ms": round(start_ms, 1)},
    )


async def test_s02_crash_recovery(engine: DBOSWorkflowEngine) -> None:
    """S-02[E2E]: 真实 SIGKILL 后恢复；从最近 durable step 续跑，recovery ≤60s（SLO-WF-02）。"""
    eid = _eid("s02")
    run_id = run_id_for("poc-durable", eid)
    tenant = "tenant-s02"
    args = {"execution_id": eid, "timer_seconds": 4.0, "approval_timeout_seconds": 1.0}
    worker = WorkerProcess(
        ["start", "--tenant", tenant, "--trace", f"trace-{eid}", "--args-json", json.dumps(args)]
    )
    try:
        worker.wait_for("STARTED", timeout=30.0)
        wait_for_condition(
            lambda: engine.step_executions(run_id).get("write_report_record") == 1,
            timeout=20.0,
            description="step1 durable commit",
        )
        worker.kill()
    finally:
        if worker.proc.poll() is None:
            worker.kill()

    killed_at = time.monotonic()
    recovery = WorkerProcess(["recover", "--run-id", run_id, "--timeout", "60"])
    recovery.wait_for("COMPLETED", timeout=70.0)
    recovery_elapsed = time.monotonic() - killed_at

    assert recovery_elapsed <= 60.0  # SLO-WF-02
    assert (await engine.get_status(run_id)).status == "SUCCESS"
    executions = engine.step_executions(run_id)
    assert executions["write_report_record"] == 1  # durable step 未重跑（从断点续跑）
    assert engine.list_records(tenant).count((run_id, "report")) == 1  # 副作用无重复

    record_evidence(
        "P-CRASH",
        passed=True,
        detail="SIGKILL 后新进程 startup recovery 续跑；已完成 step 未重执行",
        metrics={"recovery_seconds": round(recovery_elapsed, 2)},
    )


async def test_ptimer_restart_preserves_timer(engine: DBOSWorkflowEngine) -> None:
    """P-TIMER[integration]: worker 重启后 durable timer 按原始 deadline 触发（不重置）。"""
    eid = _eid("ptimer")
    args = {"execution_id": eid, "timer_seconds": 6.0, "approval_timeout_seconds": 1.0}
    worker = WorkerProcess(
        ["start", "--tenant", "tenant-ptimer", "--trace", f"trace-{eid}", "--args-json", json.dumps(args)]
    )
    t0 = time.monotonic()
    try:
        worker.wait_for("STARTED", timeout=30.0)
        t0 = time.monotonic()
        wait_for_condition(
            lambda: engine.step_executions(run_id_for("poc-durable", eid)).get("write_report_record") == 1,
            timeout=15.0,
            description="step1 committed before kill",
        )
        worker.kill()
    finally:
        if worker.proc.poll() is None:
            worker.kill()

    recovery = WorkerProcess(["recover", "--run-id", run_id_for("poc-durable", eid), "--timeout", "40"])
    recovery.wait_for("COMPLETED", timeout=50.0)
    elapsed = time.monotonic() - t0

    # timer=6s 自原始 start 起算：若重置则 ≥ 恢复点+6+1（≈10s+）；若被跳过则 <5.5s
    assert 5.5 <= elapsed <= 9.5, f"timer deadline not preserved: elapsed={elapsed:.2f}s"
    assert (await engine.get_status(run_id_for("poc-durable", eid))).status == "SUCCESS"

    record_evidence(
        "P-TIMER",
        passed=True,
        detail="kill+restart 后 timer 按原始 deadline 触发（elapsed 落在 [5.5, 9.5]s 窗口）",
        metrics={"elapsed_seconds": round(elapsed, 2), "timer_seconds": 6.0},
    )


async def test_psignal_external_approval_signal(engine: DBOSWorkflowEngine) -> None:
    """P-SIGNAL[integration]: 外部审批 signal 唤醒等待中的 workflow 并携带 payload 推进。"""
    eid = _eid("psignal")
    run_id = run_id_for("poc-durable", eid)
    started = await engine.start(
        _start_request("tenant-psignal", eid, approval_timeout_seconds=10.0)
    )
    assert started.run_id == run_id

    await engine.signal(run_id, "approve", {"approved": True, "approver": "tenant-admin"})
    result = await engine.await_result(run_id, timeout=20.0)

    assert result["approval"] == {"approved": True, "approver": "tenant-admin"}
    assert (await engine.get_status(run_id)).status == "SUCCESS"

    record_evidence(
        "P-SIGNAL",
        passed=True,
        detail="recv 等待中被 send(signal) 唤醒，payload 完整进入 workflow 结果",
    )


async def test_s03_pinned_resume(engine: DBOSWorkflowEngine) -> None:
    """S-03[integration]: resume 使用 pinned version（不 resolve latest）；retention mock 交互。"""
    engine.set_resource_version("poc-wf-def", "v1", is_latest=True)
    guard = MockRetentionGuard()
    eid = _eid("s03")
    run_id = run_id_for("poc-durable", eid)
    args = {"execution_id": eid, "pinned_version": "v1", "timer_seconds": 3.0, "approval_timeout_seconds": 1.0}
    worker = WorkerProcess(
        ["start", "--tenant", "tenant-s03", "--trace", f"trace-{eid}", "--args-json", json.dumps(args)]
    )
    try:
        worker.wait_for("STARTED", timeout=30.0)
        wait_for_condition(
            lambda: engine.step_executions(run_id).get("write_report_record") == 1,
            timeout=15.0,
            description="step1 committed before kill",
        )
        guard.acquire(
            resource_type="workflow_definition", resource_id="poc-wf-def", version="v1", run_id=run_id
        )
        with pytest.raises(RetentionBlockedError):
            guard.assert_delete_allowed(
                resource_type="workflow_definition", resource_id="poc-wf-def", version="v1"
            )  # active 引用存在 → 禁删（RULE-WF-03 语义）
        worker.kill()
    finally:
        if worker.proc.poll() is None:
            worker.kill()

    # latest 漂移到 v2 后恢复：pinned v1 不受影响
    engine.set_resource_version("poc-wf-def", "v1", is_latest=False)
    engine.set_resource_version("poc-wf-def", "v2", is_latest=True)
    recovery = WorkerProcess(["recover", "--run-id", run_id, "--timeout", "40"])
    recovery.wait_for("COMPLETED", timeout=50.0)

    run_meta = engine.get_run(run_id)
    assert run_meta is not None and run_meta["pinned_version"] == "v1"
    result = await engine.await_result(run_id, timeout=10.0)
    assert result["pinned_version"] == "v1"  # resume 未 resolve latest(v2)

    guard.release(
        resource_type="workflow_definition", resource_id="poc-wf-def", version="v1", run_id=run_id
    )
    guard.assert_delete_allowed(
        resource_type="workflow_definition", resource_id="poc-wf-def", version="v1"
    )  # run 完成、引用释放 → 放行

    record_evidence(
        "P-PIN",
        passed=True,
        detail="恢复后仍按 start 时 pinned 的 v1 执行（latest 已漂移 v2）；retention 引用生命周期成立",
    )


async def test_s04_step_timeout(engine: DBOSWorkflowEngine) -> None:
    """S-04[integration]: step timeout 上限触发定义失败策略，有界返回非无限等待。"""
    eid = _eid("s04")
    run_id = run_id_for("poc-durable", eid)
    started_at = time.monotonic()
    await engine.start(
        _start_request(
            "tenant-s04", eid, external_delay_seconds=5.0, step_timeout_seconds=0.5
        )
    )

    async def until_error() -> bool:
        while (await engine.get_status(run_id)).status != "ERROR":
            await asyncio.sleep(0.05)
        return True

    await asyncio.wait_for(until_error(), timeout=10.0)
    elapsed = time.monotonic() - started_at

    assert elapsed < 4.0  # timeout=0.5s 生效（若无限等待则 ≥ external_delay=5s）
    assert (await engine.get_status(run_id)).status == "ERROR"
    with pytest.raises(Exception):  # noqa: B017, PT011 — DBOS 透传/包装 step TimeoutError（消息格式属 DBOS 内部）
        await engine.await_result(run_id, timeout=5.0)

    record_evidence(
        "P-TIMEOUT",
        passed=True,
        detail="step timeout=0.5s 触发（external_delay=5s），workflow 有界转 ERROR",
        metrics={"elapsed_seconds": round(elapsed, 2)},
    )


async def test_s05_idempotency(engine: DBOSWorkflowEngine) -> None:
    """S-05[integration]: 同 run_id 重放为 no-op，不可逆写副作用重复次数=0（P-IDEMP）。"""
    eid = _eid("s05")
    request = _start_request("tenant-s05", eid)
    first = await engine.start(request)
    result_first = await engine.await_result(first.run_id, timeout=20.0)

    second = await engine.start(request)  # 同 execution 重放（如调用方重试）
    assert second.run_id == first.run_id
    result_second = await engine.await_result(first.run_id, timeout=20.0)
    assert result_second == result_first

    assert engine.step_executions(first.run_id)["write_report_record"] == 1
    assert engine.list_records("tenant-s05").count((first.run_id, "report")) == 1

    record_evidence(
        "P-IDEMP",
        passed=True,
        detail="同 run_id 二次 start 返回既有执行；step 未重跑、业务记录恰好 1 条",
    )


async def test_s06_scale_two_workers(engine: DBOSWorkflowEngine) -> None:
    """S-06[integration]: 2 个真实 worker 进程经 database-backed queue 分摊 work（P-SCALE）。"""
    from dbos import DBOS

    # 清理上次中断运行遗留的 ENQUEUED 行，防止被本次 worker 误认领
    purge_stale_enqueued(resolve_db_url())

    workers = [
        WorkerProcess(
            ["worker", "--index", "0", "--idle-seconds", "240"],
            extra_env={"DBOS__VMID": "worker-0"},
        )
    ]
    try:
        workers[0].wait_for("READY-0", timeout=60.0)
        # 错峰启动：避免两个 worker 同一瞬间争抢首个任务批
        time.sleep(1.5)
        workers.append(
            WorkerProcess(
                ["worker", "--index", "1", "--idle-seconds", "240"],
                extra_env={"DBOS__VMID": "worker-1"},
            )
        )
        workers[1].wait_for("READY-1", timeout=90.0)
        queue = launch_dbos()
        tag = uuid.uuid4().hex[:6]
        run_ids = [f"s06-{tag}-{i:02d}" for i in range(8)]
        handles = [
            await asyncio.to_thread(
                queue.enqueue,
                poc_durable_workflow,
                rid,
                "tenant-s06",
                f"trace-{rid}",
                "v1",
                external_delay_seconds=3.0,  # 首批 ≥3s 在飞，确保第 2 个 worker 的 poll 能认领第二批
            )
            for rid in run_ids
        ]
        for handle in handles:
            try:
                await asyncio.wait_for(
                    asyncio.to_thread(DBOS.get_result, handle.get_workflow_id()), timeout=25.0
                )
            except Exception as error:  # noqa: BLE001 — 诊断：dump worker 输出
                dumps = "\n".join(
                    f"--- worker[{i}] ---\n{w.output}" for i, w in enumerate(workers)
                )
                raise AssertionError(
                    f"get_result failed for {handle.get_workflow_id()}: {error}\n{dumps}"
                ) from error
        statuses = [
            await asyncio.to_thread(DBOS.get_workflow_status, handle.get_workflow_id())
            for handle in handles
        ]
        assert all(s.status == "SUCCESS" for s in statuses)
        executors = {s.executor_id for s in statuses}
        assert {"worker-0", "worker-1"} <= executors  # 两个 worker 都拉到了 work（P1：第 2 个 worker 生效）
        assert len(engine.list_records("tenant-s06")) >= 8  # 每个任务都有 durable 产物
    finally:
        for worker in workers:
            if worker.proc.poll() is None:
                worker.stop()

    record_evidence(
        "P-SCALE",
        passed=True,
        detail="database-backed queue + 2 真实 worker 进程；executor_id 显示双 worker 分摊",
        metrics={"distinct_executors": len(executors), "workflows": len(run_ids)},
    )


async def test_b02_tenant_isolation(engine: DBOSWorkflowEngine) -> None:
    """B-02[integration]: tenant A 的 workflow run 对 tenant B 的查询不可见（NFR-SEC-01）。"""
    eid_a, eid_b = _eid("b02a"), _eid("b02b")
    run_a = run_id_for("poc-durable", eid_a)
    run_b = run_id_for("poc-durable", eid_b)
    await engine.start(_start_request("tenant-a", eid_a))
    await engine.start(_start_request("tenant-b", eid_b))
    await engine.await_result(run_a, timeout=20.0)
    await engine.await_result(run_b, timeout=20.0)

    runs_a = engine.list_run_ids("tenant-a")
    runs_b = engine.list_run_ids("tenant-b")
    assert runs_a == [run_a] and run_b not in runs_a
    assert runs_b == [run_b] and run_a not in runs_b

    records_a = engine.list_records("tenant-a")
    assert records_a and all(run_id == run_a for run_id, _kind in records_a)
    records_b = engine.list_records("tenant-b")
    assert records_b and all(run_id == run_b for run_id, _kind in records_b)

    record_evidence(
        "B-02",
        passed=True,
        detail="tenant 维度 run/record 查询隔离；跨租户互不可见（tenant scope 由 adapter 层承载）",
    )


async def test_rpo_zero_commit(engine: DBOSWorkflowEngine) -> None:
    """RPO[integration]: step commit 后 SIGKILL，committed state 无丢失（RULE-backend-database-001）。"""
    eid = _eid("rpo")
    run_id = run_id_for("poc-durable", eid)
    tenant = "tenant-rpo"
    args = {"execution_id": eid, "timer_seconds": 5.0, "approval_timeout_seconds": 0.0}
    worker = WorkerProcess(
        ["start", "--tenant", tenant, "--trace", f"trace-{eid}", "--args-json", json.dumps(args)]
    )
    try:
        worker.wait_for("STARTED", timeout=30.0)
        wait_for_condition(
            lambda: engine.step_executions(run_id).get("write_report_record") == 1,
            timeout=20.0,
            description="step1 committed before kill",
        )
        worker.kill()
    finally:
        if worker.proc.poll() is None:
            worker.kill()

    # kill -9 后、restart 前：committed state 仍在（RPO=0）
    assert engine.step_executions(run_id)["write_report_record"] == 1
    assert (run_id, "report") in engine.list_records(tenant)

    recovery = WorkerProcess(["recover", "--run-id", run_id, "--timeout", "60"])
    recovery.wait_for("COMPLETED", timeout=70.0)

    assert engine.list_records(tenant).count((run_id, "report")) == 1  # 无重复补偿写
    assert engine.step_executions(run_id)["write_report_record"] == 1
    assert (await engine.get_status(run_id)).status == "SUCCESS"

    record_evidence(
        "RULE-backend-database-001",
        passed=True,
        detail="durable state 全量在 PostgreSQL；SIGKILL 零丢失、恢复后无重复副作用",
    )


async def test_slo_obs01_trace_correlation(
    engine: DBOSWorkflowEngine, trace_events: TraceCorrelator
) -> None:
    """SLO-OBS-01: 执行链 trace_id/run_id/tenant_id 关联完整率 ≥99%（样本自足）。"""
    for index in range(2):  # 本测试自产样本，不依赖 -k 是否选中前序测试
        eid = _eid(f"slo{index}")
        run_id = run_id_for("poc-durable", eid)
        await engine.start(_start_request("tenant-slo", eid, approval_timeout_seconds=5.0))
        await engine.signal(run_id, "approve", {"approved": True})
        await engine.await_result(run_id, timeout=20.0)
        assert (await engine.get_status(run_id)).status == "SUCCESS"

    assert trace_events.total_events >= 10  # 两条链 × (start/signal/result + 4 step 事件)
    trace_events.assert_slo_obs01()  # ≥0.99

    record_evidence(
        "SLO-OBS-01",
        passed=True,
        detail=f"trace 关联完整率 {trace_events.completeness():.4f}（{trace_events.correlated_events}/{trace_events.total_events}）",
        metrics={
            "total_events": trace_events.total_events,
            "correlated": trace_events.correlated_events,
        },
    )
