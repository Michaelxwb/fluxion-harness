...
> 11. **Step 显式声明执行模式**（ADR-062）：`execution_mode ∈ {SYNC, ASYNC}` 由 Draft 声明并与能力 `async_submittable` 合取校验；异步不是能力实现类型。
...
- **执行模式**：进入 ASYNC 分支的前提是步骤**显式声明** `execution_mode=ASYNC` 且能力 `async_submittable=true`（ADR-062）；不满足时在 Draft 校验阶段即 `CAPABILITY_ASYNC_NOT_INVOKABLE`(422)，运行期由 `CAP-LIB-03` 兜底，**不静默降级为同步**；
...
能力侧只声明 `capability_implementation.async_submittable`；`SVC-API-05` 与 `CAP-LIB-03` **合取校验**
...
| 异步语义隐含在能力实现里 | Step **显式声明** `execution_mode`，与能力 `async_submittable` 合取校验（ADR-062） |
