# 通用智能服务执行框架 V1.14 总体设计说明书

> **文档状态**：总体设计**唯一上游契约**。V1.9~V1.14 的全部裁决（ADR-033~ADR-068）已**合入本正文**；正文与 ADR 冲突时以正文为准（ADR 只保留决策理由与历史）。
> **版本**：V1.14  
> **日期**：2026-09-13  
...
>
> **V1.14 关键修订（相对 V1.8；汇总 V1.9~V1.14 全部裁决）**
>
> 1. **Builder 执行可见范围**（ADR-052）：Admin=租户全部；Builder=自建 Service 的执行 **∪** 被授权 Agent 相关的执行；越界视为不存在（`EXECUTION_NOT_FOUND`）。
> 2. **产物访问判定**（ADR-053）：一律以 `artifact` 表的 `execution_id`/`execution_step_id` FK 为准，`artifact_ids` 数组只记录重试复制来源。
> 3. **重新投递 ≠ 重新执行**（ADR-054）：`EXE-API-07` 只新建投递尝试、不重跑业务步骤；`UNKNOWN` 重投可能重复通知（产品已接受，前端二次确认）。
> 4. **步骤级人工策略**（ADR-055）：`human_policy ∈ {never, on_uncertainty, always}`，与 `failure_policy` 正交。
> 5. **集群级 Resource Class 槽位用 PG 计数器**（ADR-056 修订）：`worker_slot_counter` 恒 3 行、`resource_class` 主键；占用生命周期 = **持久占用**（ADR-068），ASYNC 提交后释放 lease 但**不释放槽位**。
> 6. **Capability 判定收敛**（ADR-057）：`risk_level` + `side_effect` 作安全审计维度，直调结论只由 `invocation_policy(DIRECT/EXECUTION_ONLY)` 派生 `direct_invocation`，谓词只定义一次。
> 7. **凭据来源单点**：`capability_implementation.auth_mode`（USER_PLATFORM/SHARED_SECRET/NONE）是唯一声明处，`config` 内出现该键一律拒绝。
> 8. **字段级授权**（ADR-058）：模型与项目平台的敏感字段（`api_key`/`extra_headers`、`auth_type`/`auth_schema`）仅 Admin 可写，非 Admin 携带即 `FIELD_ADMIN_ONLY`(403)。
> 9. **角色三条守卫**（ADR-059）：不得改自己的 `role`、提权需二次确认、禁止停用最后一名 Admin。
> 10. **运行面/管理面分离**（ADR-060）：`CH-DATA-*`/`CH-INT-*` 由 agent-runtime 承载，管理面停机不影响运行面。
> 11. **Step 显式声明执行模式**（ADR-062）：`execution_mode ∈ {SYNC, ASYNC}` 由 Draft 声明并与能力 `supported_execution_modes` 合取校验；异步不是能力实现类型。
> 12. **执行源三态**（ADR-063）：`execution_source ∈ {FORMAL, TEST, CAPABILITY_TEST}`；能力测试产生最小执行身份，产物统一走 `EXE-API-06`，授权判定保持单点。
> 13. **授权变更为单条操作**（ADR-064）：`POST .../grants` + `POST .../grants/{grant_id}/revoke`，不做集合覆盖写、不引入集合版本 CAS。
> 14. **取消一个实现、两个调用方**（ADR-065）：运行态取消统一走 `EXE-API-03`（Console=Admin 会话；IM `/stop` 经 `CH-INT-02` 调同一 Application）；`EXE-API-05` 只处理 `WAITING_HUMAN` 决策。
> 15. **投递按逻辑消息聚合**（ADR-066）：`channel_delivery.message_key` 标识逻辑消息，`delivery_status` 取有效尝试；同一逻辑消息只允许一条非终态行。
> 16. **本轮唯一化口径**（ADR-067）：提案唯一谓词含 `superseded_at IS NULL`；Builder 可见范围为并集；身份与角色以 `AUTH-API-01` 为唯一读取入口；`resource_scope_types` Owner=模块 12。
> 17. **工具链与文档基线**：`resources_scope = {type, refs[], attributes?}`；单基线迁移 0001；设计契约 ↔ 迁移 ↔ ORM 逐字段一致性为架构门禁。
>
> **核心设计原则**
>
> ```text
...
文件/Shell 能力必须通过 Workspace + Sandbox Executor；**V1 Workspace 粒度锁定为 Execution**（`owner_type=EXECUTION`，`workspace.owner_id = service_execution.id`），Conversation 级 Workspace 仅用于 Skill 调试预演，不能恢复成“每用户固定 workspace/pod”的有状态模型（ADR-035/`07-Workspace与Sandbox基线`）。
...
- Kubernetes 仅用于编排 **共享的** agent-runtime / worker / channel-gateway 副本与 HPA，**不得**为单用户/单会话创建专属 Pod；Skill 隔离由隔离子进程承担（ADR-035），不依赖每 Skill 一个容器；
- `platform-api`、`agent-runtime`、`worker`、`channel-gateway` 为四个独立运行角色；**`maintenance`（保留/清理与槽位对账）是同镜像的第五个 entrypoint**（ADR-061 N-2：独立连接池与开关，副本=1），不是第五个镜像；
...
Knowledge 表示 Agent 可以查询的专业知识源，但 **V1 将完整 Knowledge 能力划入规划项**（ADR-022）。
...
### ProjectPlatform 与 ProjectIntegration 的物理边界
...
V1 核心后台交付**四个应用镜像**；此外 `worker` 镜像带**第五个 entrypoint `maintenance`**（保留/清理与槽位对账，独立连接池与开关，ADR-061 N-2），它不单独交付镜像、不进入用户运行主链：
...
```text
新用户申请
→ platform-api 调 Kubernetes API        # 禁止
→ 创建专属 Agent Pod                    # 禁止
→ 用户绑定 Pod                          # 禁止
```
...
用户确认后（唯一可信创建路径，`EXE-LIB-01/02/03`）：

```text
Agent Runtime (CH-INT-02 CONFIRM / 文本确认)
→ ExecutionService.issue_proposal / confirm_proposal
→ 同一事务：校验 confirmation_ref|digest 归属与有效期 → 复用 proposal.snapshot_id
   → 写 execution_proposal(CONFIRMED) + service_execution + root step + command 幂等行
→ PostgreSQL → （提交后）Worker wake hint
```

管理员手工发起（测试/重试等）：

```text
platform-api → 同一个 ExecutionService（同一实现，不复制业务逻辑）
```

这样保证可信执行边界只有一份实现，同时 `platform-api` 不成为普通用户执行链的同步依赖。

**同一实现、两个调用方**（ADR-065）：取消、重试、人工决策都只有一份 Application 实现；Console 走 `/api/v1/*`，IM 走 `CH-INT-02`，授权在 Application 层按可信 principal 判定（Admin=租户内；runtime=只能是本人）。**不新增 `/internal/v1/executions/*/cancel` 这类同义端点。**
...
`pagination_type ∈ {PAGE, OFFSET, CURSOR}`，**缺省 CURSOR**（模块 07 `contract:capability-implementation-schema` 的 `data_retrieval_policy`）。
...
→ items == [] 固定兜底（PAGE/OFFSET）
→ len(items) < page_size 仅当短页终止被显式开启（short_page_terminates=true）才可作为结束
```

同时强制：`max_pages / max_items / max_duration_seconds / duplicate_page_detection`。**短页终止默认开启（`short_page_terminates=true`）**；下游可能产生中间短页时必须允许显式关闭，只有保证「非最后页必满」才可安全依赖它（V1.12 裁决）。不得用 `total` 作为结束依据。
...
erDiagram
    ...
    SERVICE_EXECUTION {
        ...
        string execution_source "FORMAL|TEST|CAPABILITY_TEST"
        string execution_mode "SYNC|ASYNC"
        string status "9 states"
        string current_step
        string requested_action "RESUME|CANCEL NULL"
        datetime human_deadline
        datetime next_run_at
        string lease_owner
        datetime lease_expires_at
        bigint lease_epoch "fencing"
    ...
    EXECUTION_SNAPSHOT {
        uuid id PK
        uuid service_id FK
        string source "FORMAL|TEST"
        uuid service_release_id FK
        bigint draft_revision
        string test_mode
        string content_hash
        json snapshot_json "frozen projection"
        string snapshot_ref
        datetime created_at
    }
    ...
    EXECUTION_PROPOSAL {
        ...
        uuid service_release_id FK
        uuid snapshot_id FK
        string confirmation_digest "content binding"
        string status "PENDING|CONFIRMED"
        datetime superseded_at "NULL = current"
        datetime expires_at
    ...
    EXECUTION_STEP {
        ...
        string step_type
        string execution_mode "SYNC|ASYNC"
        string slot_resource_class "ADR-068 NULL"
        string operation_id "logical call identity"
        string status
        int attempt
    ...
    CHANNEL_DELIVERY {
        ...
        string channel
        string event_type
        string event_id "attempt identity"
        string message_key "ADR-066 logical message"
        string status
        int attempt
        bigint lease_epoch
        string dedupe_key
    ...
    WORKER_SLOT_COUNTER {
        string resource_class PK "ADR-056"
        int used
        int slot_limit
    }
    ...
    EXECUTION_PROPOSAL ||--|| EXECUTION_SNAPSHOT : freezes
    EXECUTION_PROPOSAL ||--o| SERVICE_EXECUTION : confirms_into
    EXECUTION_SNAPSHOT ||--|| SERVICE_EXECUTION : freezes
    SERVICE_EXECUTION ||--o{ EXECUTION_STEP : contains
    SERVICE_EXECUTION ||--o{ CHANNEL_DELIVERY : notifies
    SERVICE_EXECUTION ||--o{ ARTIFACT : owns_for_authorization
...
UNIQUE(tenant_id, parent_execution_id) WHERE parent_execution_id IS NOT NULL
CHECK execution_source ∈ {FORMAL,TEST,CAPABILITY_TEST} 且三分支字段形状受控（ADR-063）
INDEX(status, next_run_at)
INDEX(status, lease_expires_at)
INDEX(actor_user_id, created_at)
INDEX(trace_id)
INDEX(tenant_id, capability_id, created_at)

execution_proposal:
UNIQUE(tenant_id, conversation_id) WHERE status='PENDING' AND superseded_at IS NULL AND is_deleted=false
CHECK status='CONFIRMED' ⇔ confirmed_message_id/confirmed_at/execution_id 全非空（ADR-049/ADR-067）

execution_step:
UNIQUE(tenant_id, execution_id, step_key)
INDEX(tenant_id, execution_id, sequence_no)
INDEX(tenant_id, status, wait_until, is_deleted)
INDEX(slot_resource_class, status)   -- ADR-068 持久占用对账

channel_delivery:
UNIQUE(tenant_id, dedupe_key)
INDEX(tenant_id, message_key, attempt)   -- ADR-066 逻辑消息聚合

worker_slot_counter:
PRIMARY KEY(resource_class)；恒 3 行；CHECK used BETWEEN 0 AND slot_limit（ADR-056）
...
- config
- execution_policy{deadline_seconds,max_retries,backoff_seconds}
...
workspace
- id / tenant_id
- owner_type(EXECUTION)        # V1 锁定 Execution；Conversation 仅 Skill 调试预演
- owner_id                     # = service_execution.id
- backend_type                 # SANDBOX（V1 唯一生产后端，ADR-035）
- workspace_ref                # 模块 13 解析出的受控路径/句柄，不向 Runtime 暴露宿主路径
- status(ACTIVE|EXPIRED|RELEASED)
- quota_bytes
- create_time / update_time / expires_at
```

UNIQUE (tenant_id, owner_type, owner_id) WHERE is_deleted=false。Workspace **粒度只到 Execution**，不存在 turn / skill invocation 级 Workspace（P1-8 裁决）。
...
唯一 durable 执行根（模块 05 拥有）；Chat turn 不建本表（见 4.3.2）。

```text
service_execution
- id / tenant_id
- service_id            # FORMAL/TEST 必填；CAPABILITY_TEST 必须为空
- capability_id         # 仅 CAPABILITY_TEST 必填（ADR-063）
- service_release_id    # FORMAL 必填；TEST/CAPABILITY_TEST 为空
- actor_user_id
- primary_agent_id      # CAPABILITY_TEST 为空
- conversation_id / delivery_route_id / workspace_id
- snapshot_id           # FORMAL/TEST 必填；CAPABILITY_TEST 为空
- resource_scope_json / input_json
- execution_source      # FORMAL | TEST | CAPABILITY_TEST（ADR-063）
- execution_mode        # SYNC | ASYNC，由编译结果派生，不接受调用方传入（ADR-062）
- status
- waiting_reason / current_step（业务可读名按 ADR-051 在读取期从 snapshot/进度事件派生，不落列）
- human_deadline / human_decision_at / requested_action / context_summary
- next_run_at / lease_owner / lease_expires_at / lease_epoch（fencing）
- attempt / max_attempts / max_retries / retry_count / priority
- idempotency_key / trace_id / channel_source
- parent_execution_id / root_execution_id（重试派生）
- result_ref / error_code / error_message
- create_time / update_time / started_at / finished_at / cancel_requested_at
```
...
PENDING
RUNNING
WAITING            # 仅时间等待（WAIT 步骤 / 轮询）
WAITING_HUMAN      # 人工检查点，决策写入走 EXE-API-05
RETRY_WAIT
CANCELLING
SUCCEEDED
FAILED
...
**执行源三分支条件约束**（ADR-063，`contract:execution-source-check`）：

```sql
(source='FORMAL'          AND service_id IS NOT NULL AND capability_id IS NULL
                          AND snapshot_id IS NOT NULL AND service_release_id IS NOT NULL
                          AND draft_revision IS NULL AND test_mode IS NULL)
OR (source='TEST'         AND service_id IS NOT NULL AND capability_id IS NULL
...
OR (source='CAPABILITY_TEST' AND service_id IS NULL AND capability_id IS NOT NULL
                          AND snapshot_id IS NULL AND service_release_id IS NULL
                          AND draft_revision IS NULL AND test_mode IN ('DRY_RUN','REAL_TEST'))
```

**取消与决策**（ADR-065）：运行态取消只有 `EXE-API-03` 一个实现（Console=Admin 会话；IM `/stop` 经 `CH-INT-02` 调同一 Application，授权按可信 principal 判本人）；`EXE-API-05` 只在 `status=WAITING_HUMAN` 时接受 `decision ∈ {RESUME, CANCEL}`，写入 `requested_action`/`human_decision_at`。两条入口在 WAITING_HUMAN 上语义等价，收敛到同一 `execution_command(CANCEL)`。
...
- step_key / sequence_no
- step_type                  # AGENT | CAPABILITY | WAIT | HUMAN | DELIVERY
- execution_mode            # SYNC | ASYNC —— 来自 Draft 显式声明（ADR-062）
- slot_resource_class       # browser | external-scan | large-report | NULL（ADR-068 持久占用标记）
- operation_id              # 首次创建生成，重试/派生沿用；副作用键 effect:{operation_id}
- source_step_id            # 重试复制的来源步骤；checkpoint_ref（图恢复点）
- status                    # PENDING | RUNNING | WAITING | SUCCEEDED | FAILED | CANCELLED | SKIPPED
- attempt
- input_json / output_json / result_ref
- wait_until
- idempotency_key
...
**ASYNC 步骤的持久占用**（ADR-068）：提交成功后 root 置 WAITING 并**释放 lease**，但 `slot_resource_class` 保持到步骤终态（含对账收敛）才清空并 `used-1`——槽位是下游压力配额，lease 是调度所有权，两者释放时机不同；maintenance 对账按"持久占用"统计，否则会突破集群并发上限。

### ExecutionSnapshot

单行复用的不可变业务投影（`service_execution.snapshot_id = proposal.snapshot_id`，不双写大 JSONB）：

```text
execution_snapshot
- id / tenant_id
- service_id / source(FORMAL|TEST) / service_release_id
- draft_revision / test_mode            # TEST 必填
- content_hash                          # 覆盖 revision/test_mode/全部业务投影
- snapshot_schema_version               # 未知版本拒绝执行
- snapshot_json                         # CORE-LIB-05 ExecutionProjection 完整冻结
- snapshot_ref                          # 超大制品的对象引用
```

`snapshot_json` 内冻结：service/agent 版本与内容 hash、model config snapshot、skill artifact checksum、knowledge binding refs、capability contract names、业务阶段/确认点/执行逻辑、delivery route ref、trace_id。**能力测试（CAPABILITY_TEST）不产生快照行**（ADR-063）。

UNIQUE (tenant_id, service_id, source, content_hash) WHERE service_id IS NOT NULL；未知 `snapshot_schema_version` → `SERVICE_VALIDATION_FAILED`，不静默按新结构解析。
...
`/bind` 只把外部用户身份映射为 PlatformUser。**唯一通路是 Gateway → agent-runtime 的 `CH-INT-02`（`POST /internal/v1/commands`，action=…由模块 10 处理），禁止 Gateway 调用 platform-api 的业务写路径**（ADR-033/ADR-060：运行面与管理面分离；Control Plane 停机不影响绑定链路）：

```mermaid
sequenceDiagram
    participant U as User
    participant G as Channel Gateway
    participant R as agent-runtime (CH-INT-02)
    participant DB as PostgreSQL
    U->>G: /bind CODE
    G->>R: verified envelope + action=BIND（内部 JWT，AUTH-LIB-02）
    R->>DB: validate bind_code
    R->>DB: upsert ChannelIdentity + ChannelBinding
    R->>DB: bind_code = USED
    R-->>G: platform_user_id
    G-->>U: 绑定成功
```
...
Agent 只能提交不含身份的 `ExecutionProposalCandidate`；Runtime 经 `EXE-LIB-02` **签发待确认提案**：

```text
identity resolve（tenant/actor/conversation 只来自可信 ctx，不接受 candidate 提供）
→ Service 已发布且 enabled
→ Resource Scope validation（type 命中部署白名单 / refs / attributes）
→ input schema validation
→ 渲染 confirmation.summary_template（确定性渲染器，禁止 LLM 参与）→ rendered_summary
→ 冻结 CORE-LIB-05 投影 → 写 execution_snapshot
→ 同事务：同会话旧 PENDING 置 superseded_at=now + 插入 execution_proposal
→ 落 ASSISTANT message(role=ASSISTANT, message_type=PROPOSAL)
```

**当前提案的唯一判定**（ADR-067）：`status='PENDING' AND superseded_at IS NULL AND is_deleted=false` —— 唯一部分索引与"取回当前提案"查询共用该谓词，被取代的行即使 `status` 仍为 PENDING 也不再占用唯一槽位（同会话可连续签发替换）。

用户确认时（`EXE-LIB-03`，事务内）：校验 `confirmation_ref`/`confirmation_digest`、归属、`expires_at`、`current release` 未变 → 调用 `EXE-LIB-01` 复用 `proposal.snapshot_id` 创建执行 → 以 `proposal:{proposal_id}` 为幂等键，已 CONFIRMED 返回原执行。
...
状态机唯一事实源是模块 05 §3.3（九态）；本图仅示意。补充三条 V1.14 语义：

- **执行模式**：进入 ASYNC 分支的前提是步骤**显式声明** `execution_mode=ASYNC` 且能力 `supported_execution_modes` 包含 `ASYNC`（ADR-062）；不满足时在 Draft 校验阶段即 `CAPABILITY_ASYNC_NOT_INVOKABLE`(422)，运行期由 `CAP-LIB-03` 兜底，**不静默降级为同步**；
- **槽位**：ASYNC 提交成功、root 置 WAITING 后**释放 lease 但保留 `slot_resource_class`**（ADR-068），提前释放会突破集群并发上限；
- **取消/决策**：`cancel_requested_at` 优先于 `human_deadline`；`WAITING_HUMAN` 的决策写入只走 `EXE-API-05`，其余状态取消只走 `EXE-API-03`（ADR-065）。
...
next_run_at = ...
lease = NULL
slot_resource_class = 保留（ASYNC 步骤持久占用）
```
...
eligible =
 ...
 OR (status='WAITING_HUMAN' AND human_deadline <= now())
...
恢复时按 `operation_id` 查 `async_task_run`：

```text
SUBMITTED / RUNNING            → 禁止再次 submit；继续 status/result 轮询
SUBMITTING / SUBMITTED_UNKNOWN → 只对账（CAP-LIB-03 reconcile），不盲目重发
终态                            → 复用结果
```

外部提交在 `SUBMITTING` 状态下崩溃时，视为 `SUBMITTED_UNKNOWN` 并由对账收敛；对账到 `reconcile_deadline` 仍未知 → `FAILED(SUBMISSION_UNCONFIRMED)`（不伪造外部 task ID）。跨重试派生**沿用同一 `operation_id` 与 `effect:{operation_id}`**，禁止以新 execution_id 生成新的外部提交键。
...
### ExecutionProposalCandidate（Agent → Runtime，`EXE-LIB-02` 入参）
...
```json
{
  ...
  "evidence": {"source_message_id": "msg-xxx"}
}
```

`resource_scope` 收敛形态 `{type, refs[], attributes?}`（ADR-050/067）：`type` 必须命中当前部署 Integration manifest 的 `resource_scope_types` 白名单（内存投影、装配期校验、不落库），`attributes` 只允许该类型声明的字段；空白名单时不提交伪造 `type`。

**服务端（可信上下文）补充，候选体不得携带**：

```text
tenant_id / actor_user_id / conversation_id
delivery_route_id（取本会话当前持久 DeliveryRoute，此后不随会话漂移，ADR-045）
service_release_id（签发时冻结）
snapshot_id（CORE-LIB-05 投影冻结为单快照行）
execution_source=FORMAL / execution_mode（由编译结果派生）
```

签发返回 `ExecutionProposalView`（含 `proposal_id`、`rendered_summary`、`confirmed_facts`、`confirmation_ref`、`expires_at`）；`rendered_summary` 是本轮展示文本，**不持久化为业务事实**（`execution_proposal` 无该列），`confirmation_digest` 绑定结构化事实（tenant/actor/release/input/scope/template_hash/expires_at）。
...
### Capability Data Retrieval Policy 合同
...
```json
{
  ...
  "termination": {"empty_items":true,"short_page_terminates":true,"use_total":false},
  "limits": {"max_pages":200,"max_items":20000,"max_duration_seconds":120,"duplicate_page_detection":true}
}
```

`empty_items=true` 是 PAGE/OFFSET 的**固定兜底**结束条件；`short_page_terminates` 默认 true，仅当“非最后页必满”成立时才可依赖，下游可能产生中间短页时必须显式关闭；`use_total` 不参与结束判定。

### Async Capability 统一合同

ASYNC 不是能力实现类型，而是 **Service Step 的显式 `execution_mode` 声明**（ADR-062）：

```text
Draft.steps[].execution_mode = ASYNC（仅 type=CAPABILITY）
  → Worker 经 CAP-LIB-03 submit_async(ctx, capability_key, input, idempotency_key=effect:{operation_id})
  → 外部任务生命周期由 async_task_run 持久承载（SUBMITTING/SUBMITTED_UNKNOWN/SUBMITTED/RUNNING/SUCCEEDED/FAILED/CANCELLED）
  → 轮询/对账由 WORK-LIB-05 有界推进（max_poll_attempts / reconcile_deadline）
```

能力侧声明 `capability_implementation.supported_execution_modes` 非空模式集合；步骤模式必须属于集合；`SVC-API-05` 与 `CAP-LIB-03` **合取校验**
（能力不支持异步而步骤声明 ASYNC、或能力仅支持异步而步骤声明 SYNC，均返回
`CAPABILITY_ASYNC_NOT_INVOKABLE`(422)，不静默降级）。**Skill/Agent 直调路径不得提交异步任务**，
命中即得同一错误码；不同业务系统的取数细节由 Adapter 封装，Service/Skill 不自己写 polling framework。
...
### Worker Command（`execution_command`）

`command_type ∈ {CANCEL, RESUME}` 由 Worker 应用；`RETRY` 由 `EXE-API-04` 在 API 事务内直接标 APPLIED（Worker 不重复应用）。

```text
CANCEL   ← EXE-API-03（运行态，Admin 会话或 runtime 本人）| EXE-API-05（WAITING_HUMAN 决策）| CH-INT-02 STstop
RESUME   ← EXE-API-05（WAITING_HUMAN 决策）| CH-INT-02 RESUME
RETRY    ← EXE-API-04 同事务完成（result_execution_id 指向新执行，不进入 Worker 循环）
```

`payload_json` 承载决策细节（`comment` 等）；`status ∈ {PENDING, APPLIED, REJECTED}`；`idempotency_key` 唯一（软删不占位）。应用前必须持有效 `lease_epoch`（fencing），所有写入与 command APPLIED 在同一事务。
...
{
  "delivery_id": "d-1",
  "attempt_token": "<由 Python Channel Application 签发，绑定 tenant/delivery_id/lease_epoch/过期时间>"
}
```

**路由不可由调用方指定**（`01-架构与规范/06-接口设计基线` §10）：投递路由取自 `channel_delivery.delivery_route_id`（创建 Execution 时由可信上下文固定写入），调用方只提交 `delivery_id` + 限时 `attempt_token`。可伪造 `route_id` 即越权漏洞，V1 不提供该字段。


**发送许可与进程边界**（ADR-071 修订）：Worker 领取后置 SENDING 并产生 epoch；Gateway 不直连 PG，发送前调用 agent-runtime 的 CH-DATA-03 原子消费许可（`permit_consumed_at IS NULL → CURRENT_TIMESTAMP`），提交后取得冻结 route/payload，才执行 CH-INT-01 的单次发送。重复/过期/旧 epoch 拒绝；许可响应丢失或发送结果未知按 UNKNOWN 处理，不盲目重发。SENDING 不是许可已消费的证明。
...
**生产沙箱 = `SandboxExecutor` 的隔离子进程实现**（ADR-035，V1 唯一生产后端）：独立 user/mount/PID/IPC/network namespace、non-root + `no_new_privs` + seccomp、最小只读 rootfs、默认无外部网络路由、cgroup v2 + rlimit、超时 SIGKILL + 进程组清理。`LocalSandboxExecutor`（进程内）仅用于开发/受信环境，**生产部署禁用该路径**。容器化后端留 V2 演进，SandboxExecutor SPI 不变——**不得**把“每 Skill 一个容器”写成 V1 生产方案。
...
ORDER BY priority DESC, next_run_at ASC NULLS FIRST, create_time ASC
FOR UPDATE SKIP LOCKED
LIMIT :batch;
```

claim 后原子更新（`lease_epoch + 1` 即 fencing token）：

```text
lease_epoch = lease_epoch + 1
lease_owner = worker_instance_id
lease_expires_at = now + lease_ttl
status = RUNNING          -- CANCELLING / WAITING_HUMAN 保留原状态
```

长步骤执行期间 heartbeat 延长 lease；**所有状态写入都必须比较 `lease_owner + lease_epoch` 且未过期**，失配即 `LEASE_LOST`（CORE-LIB-08）。`WAITING_HUMAN` 只处理已接受决策与超时，不重复领取（不 busy-loop）。

### 集群级 Resource Class 槽位（ADR-056 修订 / ADR-068）

```sql
-- 抢占：同事务，原子
UPDATE worker_slot_counter SET used = used + 1
 WHERE resource_class = :rc AND used < slot_limit;
-- rowcount = 0 → 本轮跳过并按 next_run_at 退避

-- 释放：步骤「持久占用」结束时
UPDATE worker_slot_counter SET used = used - 1
 WHERE resource_class = :rc AND used > 0;
```

- 三类集群级槽位：`browser` / `external-scan` / `large-report`（其余 class 进程内计数）；
- 抢占成功的同一事务写 `execution_step.slot_resource_class`；
- **释放时机 = 持久占用结束**：SYNC=终态；ASYNC=含对账收敛的终态（提交成功后释放 lease 但**不释放槽位**，ADR-068）；
- maintenance 对账口径：`used :=` ①仍持有效 lease 的 RUNNING 步骤 + ②`execution_mode='ASYNC'` 且状态非终态的步骤；泄漏方向只会少用。
...
**运行态取消只有一个实现**（ADR-065）：`EXE-API-03` `POST /api/v1/executions/{id}/cancel`。

```text
Console（Admin 会话）        → EXE-API-03
IM /stop（END_USER 本人）    → CH-INT-02 STOP → agent-runtime → 同一 Application
不新增 /internal/v1/executions/*/cancel 端点
```

可取消状态集合：`PENDING`/`RUNNING`/`WAITING`/`WAITING_HUMAN`/`RETRY_WAIT`；`CANCELLING` 幂等返回；终态 `EXECUTION_TERMINAL`(409)。授权按调用方分别判定（Admin=租户内；runtime=只能是执行 `actor_user_id` 本人）。

`WAITING_HUMAN` 的人工决策另走 `EXE-API-05`（`decision ∈ {RESUME, CANCEL}`），两条入口在该状态上语义等价，收敛到同一 `execution_command(CANCEL)`：
...
WAITING_HUMAN
waiting_reason
context_summary       # 进入等待时固化的 U03 六要素（含 completed_steps[] 与 options[{action,label,meaning}]）
human_deadline        # 必填，进入等待时间 + human_deadline_seconds（默认 86400）
requested_action      # RESUME | CANCEL；**决策被事务接受后才写入**，进入等待时为 NULL
requested_action 覆盖的两种来源：
  - EXE-API-05（Console，Admin）：decision=RESUME|CANCEL
  - CH-INT-02 RESUME/CANCEL（IM，本人）
```

**决策与超时竞争的唯一收敛**（RULE-SVC-08）：进入 WAITING_HUMAN 的同一事务写 `context_summary` + `human_deadline` + `next_run_at = human_deadline`，并**预建恰好一条 `channel_delivery(event_type='human_wait')`**（ADR-040）。截止前被事务接受的决策优先；无有效决策且 `now >= deadline` → `FAILED(HUMAN_TIMEOUT)`；相反决策 `HUMAN_DECISION_CONFLICT`；重复请求幂等返回原结果。

步骤级策略 `human_policy ∈ {never, on_uncertainty, always}`（ADR-055）与 `failure_policy` 正交：`always` 由编译器在该步骤**之前**插入 HUMAN 检查点；`on_uncertainty` 只在语义不确定/业务拒绝/达自动恢复上限时转人工，技术故障不得转人工。

User/Admin 决策写数据库后由 Worker 应用：
...
```text
V1 必须实现（RULE-WORK-06）：
global worker concurrency            # WORKER_GLOBAL_CONCURRENCY
per-capability concurrency           # 单 Worker 进程内计数
step timeout                         # WORKER_STEP_TIMEOUT_MS
集群级 Resource Class 槽位           # browser / external-scan / large-report → PG 计数器（ADR-056/068）
priority                             # claim 顺序 priority DESC, next_run_at ASC
...
Framework Core 正式保留以下扩展点（`ProviderKind` 的 V1 取值 = **4 个可替换 Provider**）：
...
基础设施 Port（同镜像内实现，不经 Integration 注册，避免"两套 Secret 解析来源"）：
  ArtifactStore / SecretProvider / WorkspaceStore+WorkspaceManager
  SandboxExecutor（ADR-035：生产= 隔离子进程）
  AgentExecutor / StepExecutor
```

**KnowledgeProvider 不在 V1 ProviderKind 白名单内**（ADR-022：Knowledge 为规划项）——需要接入时再放开并同步 Port 装配规则。V1 不建设动态 Marketplace/复杂 Plugin Lifecycle，扩展一律通过部署时装配与 Registry 注册（同 key 重复注册 fail-fast）。
...
### Knowledge（规划项，V1 不放开）

**V1 不定义 Knowledge Runtime、不注册 KnowledgeProvider、不决策向量库/切分/Embedding**（ADR-022）。放开条件：真实外部知识库的权限模型与检索 Contract 明确后，在模块 16 **整章新增**（DB/接口/Provider/Console 页同时落），并按 §5.4 的 ProviderKind 白名单流程放开。

刻意不列实现候选（`pgvector`/MCP/外部 API 等）：列出候选会让实现者以为其中某个已被选中。
...
3. AuthProvider（如需要）
4. ChannelAdapter（如项目需要新渠道）
5. Integration Test

Knowledge Provider 不在 V1 清单内（ADR-022 规划项；需要时整章新增并按 §5.4 的 ProviderKind 白名单放开）。
```
...
→
S6 AuthProvider（KnowledgeProvider 不在 V1 里程碑内）
→
S7 ExecutionService
→
...
### Gate L：Knowledge 未开放（V1 不适用）

V1 **不设 Knowledge Gate**：该能力是规划项（ADR-022），没有 DB/接口/Provider，因此没有可验收对象。放开时本 Gate 连同模块 16 一起新增，验收点为"替换 Provider 不改 Agent Runtime/Worker Core，且 ExecutionStep 记录真实 retrieval evidence"。
...
| 执行可见范围按"自建或授权"含糊表述 | **并集**：自建 Service 的执行 ∪ 被授权 Agent 相关的执行，越界 404（ADR-052） |
| 产物访问以 `artifact_ids` 数组为准 | 一律以 `artifact` 表 `execution_id`/`execution_step_id` FK 为准（ADR-053） |
| 一个「重试」按钮（重跑与重投混用） | 拆分「重新执行」(EXE-API-04) 与「重新投递」(EXE-API-07)（ADR-054） |
| 人工介入只有失败后 MANUAL | 新增步骤级 `human_policy ∈ {never,on_uncertainty,always}`，与失败策略正交（ADR-055） |
| 槽位在单个 Worker 进程内计数 | 集群级 PG 计数器（`worker_slot_counter`，3 行），占用按**持久占用**对账（ADR-056/068） |
| Capability 直调由三字段组合判定 | 直调结论只由 `invocation_policy` 派生 `direct_invocation`，谓词只定义一次（ADR-057） |
| 授权/凭据要求写在 capability definition | 凭据来源唯一由 `capability_implementation.auth_mode` 声明 |
| 模型/平台敏感字段整接口 Admin-only | 字段级授权：敏感字段仅 Admin，其余 Builder 可写（ADR-058） |
| 角色可自由改写 | 三条守卫：不得改自己 role、提权二次确认、禁止停用最后一名 Admin（ADR-059） |
| CH-DATA/CH-INT 依赖 platform-api | 由 agent-runtime 承载，运行面与管理面分离（ADR-060） |
| 异步语义隐含在能力实现里 | Step **显式声明** `execution_mode`，与能力 `supported_execution_modes` 合取校验（ADR-062） |
| 执行源只有 FORMAL/TEST | 三态 FORMAL/TEST/**CAPABILITY_TEST**，能力测试有最小执行身份（ADR-063） |
| 授权编辑=整体集合 PUT（无乐观锁） | 单条 `POST .../grants` + `.../{grant_id}/revoke`，带 `idempotency_key`（ADR-064） |
| 取消两套入口/两套授权 | 一个实现两个调用方：EXE-API-03（运行态）+ EXE-API-05（WAITING_HUMAN 决策）（ADR-065） |
| 投递汇总按"全成功才 DELIVERED" | 按 `message_key` 取**有效尝试**聚合，重投成功即收敛（ADR-066） |
| 提案唯一性只看 status | 谓词含 `superseded_at IS NULL`，"当前提案"唯一（ADR-067） |
| 保留期 REPORT 730 天等特例 | 统一跟随其 Execution 的 365/30 天 |
| 迁移 expand-contract 双读双写 | V1 破坏性变更**直接 DDL + 离线回填**，无双读兼容期（无生产数据） |
| `08/09` 汇总索引与模块双源 | 模块 `§3.3/§3.4` 是唯一 Owner；`08/09` 为**机械生成**的索引，逐字段一致是架构门禁 |
...
| V1.8 | 交互/Skill SDK/ProjectPlatform/Channel/分页决策合入正文 | 关键修订见 `docs/05-变更记录/` | 与 V6 Playbook 对齐 | Superseded |
| V1.9~V1.11 | Skill 开发模式收敛、详细设计恢复、cf-task-align 重生成、模块分档拆分与 18/19 模块新增 | 详见 `docs/05-变更记录/` | 下游基线 | Superseded |
| V1.12~V1.13.1 | 多轮 Review 裁决修复（字段级授权、产物访问、重新投递、`human_policy`、集群槽位、Capability 判定收敛、角色守卫、运行面数据通路） | 详见 `docs/05-变更记录/07~10` 与 ADR-040..060 | 已合入本正文 | Superseded |
| **V1.14** | **最简重设（N-1~N-5）+ 第五轮 Review 契约裁决（D1~D16/ADR-062..068）+ 全库 Schema 对齐** | **全部裁决已合入本正文**（见文首 17 条）；模块设计 `§3.3/§3.4` 为接口与表字段的唯一 Owner，`08/09` 为机械生成的索引 | `docs/README.md` V1.14 | **Current** |
...
**Knowledge 是规划项（ADR-022）：V1 不提供 Console 菜单、不冻结 API/DB/Provider**，Core 图、扩展点表与菜单都不得出现该入口——需要时整章新增，不用"占位菜单"表达。
...
用户可见命令（Playbook §4.4 的 6 个）：

```text
/bind <code>    外部身份绑定（Gateway 本地处理，只建 ChannelIdentity → PlatformUser）
/skills         当前 Agent 的 Skill 列表，按 platform_label 分组
/new            新建会话，不改变用户/Agent/Skill/IM 关系
/stop           取消当前会话唯一非终态 Execution（→ CH-INT-02 STOP → EXE-API-03 同一实现）
/result [id]    主动取件：重取产物/结果（→ EXE-API-06 的 Artifact Application）
/memory ...     记忆列表/清理（MEM-LIB-02）
```

内部动作（`CH-INT-02` 的九条映射，不新增用户可见命令）：

```text
SKILLS / NEW / STOP / RESUME / CANCEL / MEMORY_LIST / MEMORY_CLEAR / RESULT / CONFIRM
```

RESUME/CANCEL 走 `EXE-API-05` 同一 Application（仅 `WAITING_HUMAN` 且本人）；CONFIRM 走 `EXE-LIB-03`（`verified_message_id` 必须是实际 USER 确认事件）。**不接受"固定 4 个命令"的旧表述。**
...
13. **生产默认四个镜像：platform-api、agent-runtime、worker、channel-gateway**（Console 静态资源并入 platform-api）；**运行角色共五个**——`maintenance` 与 worker 同镜像、不同 entrypoint（ADR-061 N-2）。
