...
erDiagram
    ...
    SERVICE_RELEASE { uuid id PK uuid service_id FK string release_id string content_hash datetime published_at }
    SKILL_DEFINITION { uuid id PK string key UK string name string platform_label uuid current_artifact_id FK string status }
    SKILL_ARTIFACT { uuid id PK uuid skill_id FK string package_ref string checksum string sdk_version string entrypoint json capability_dependencies datetime created_at }
    CAPABILITY_DEFINITION { uuid id PK string name UK string side_effect string risk_level boolean enabled }
    CAPABILITY_IMPLEMENTATION { uuid id PK uuid capability_id FK string impl_type uuid project_platform_id FK json config json data_retrieval_policy int revision boolean enabled }
...
- id
- capability_id
- impl_type(PLATFORM_SERVICE/HTTP/MCP/SANDBOX)
...
异步任务不作为 impl_type；分页策略属于 Implementation。
...
SDK V1 保持薄接口：`ctx.capability.call / ctx.logger / ctx.artifact / ctx.user / ctx.session`。
