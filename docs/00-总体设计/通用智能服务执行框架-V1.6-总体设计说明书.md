# 通用智能服务执行框架 V1.7 总体设计说明书

> **文档状态**：总体设计基线（V1.7 整改对齐已合入正文，V1.6 原文见 git 历史）  
> **版本**：V1.7  
> **日期**：2026-09-10  
...
> **首个参考业务需求**：《MSS 智能服务交付平台——用户场景与 Playbook 旅程设计 V5》  
> **历史工程参考**：
...
| Project Integration | 某个项目接入框架所提供的 Service、Agent、Capability Adapter、AuthProvider、KnowledgeProvider 等集合 |
| Platform User | 框架统一用户主体，是授权、Memory、Conversation、Execution 的用户根 |
| Channel Identity | WeCom、WebChat、Slack、Mattermost 等 Channel 中的外部身份，仅映射到 Platform User |
...
| Skill | 可复用的 Agent 方法、知识、指令、判断规则、模板等资源 |
...
```text
End User
→ 使用 Agent/Service、确认、查看进度、接收结果

Admin
→ 管理用户、授权、渠道、发布、运行观测和排障

Builder / Integration Developer
→ 创建 Agent/Service
→ 配置 Skill/Knowledge
→ 定义 Capability Contract
→ 接入业务 Adapter/AuthProvider
→ 测试并发布
```

具体项目的“业务能力接入开发者”属于 Builder 工作模式，不增加第四类一级用户。
...
```text
Service Builder
→ 创建 Agent/Service，配置 Prompt/Skill/Knowledge/Capability

Integration Developer
→ 实现 CapabilityProvider/AuthProvider/KnowledgeProvider/ChannelAdapter
```

简单业务接入优先配置化；复杂接入再写 Adapter。
...
同一 SOP 可以被实现为 Agentic、Deterministic 或 Hybrid。
...
Knowledge 表示 Agent 可以查询的专业知识源。

通过 `KnowledgeProvider` 统一接入：
...
Agent Definition 只绑定逻辑 Knowledge Source，不绑定具体向量数据库实现。
...
具体项目通过 Provider/Adapter 将它映射到已有业务系统。
...
创建 Agent
→ 创建 Service
→ 声明 Capability Contract
→ 选择已有 Provider 或实现项目 Adapter
→ 配置 AuthProvider / Knowledge Source
→ Test
→ Publish
→ End User 可用
```
...
Service Definition
Skill
Knowledge Source + Provider SPI
Capability Contract + Provider SPI
Built-in Sandbox Capability（filesystem/shell）
Workspace Contract + Sandbox Executor SPI
AuthProvider SPI
Channel Gateway + Adapter SPI
...
risk_level / side_effect / execution_characteristic
Audit / Trace
```
...
| `channel-gateway` | Node.js/TypeScript | 1+ | 是 | 多 Channel 连接、消息标准化、路由、流式回复、主动投递 |
...
Capability Runtime 是 Framework Core 的主要业务扩展边界。
...
Project SDK Adapter(optional)
AuthContextResolver
```

统一接口只接受 Capability Contract + TrustedExecutionContext，不允许 Service 直接依赖项目 SDK/URL。

Agent Runtime 与 Worker 共用同一套 Capability 解析、授权和执行逻辑。

简单 HTTP/API 接入优先配置化；只有复杂认证、协议、聚合或结果转换需要项目代码 Adapter。


### 4.1.7 Auth Runtime / AuthProvider

Framework Core 不假设外部系统使用哪种认证方式。

统一扩展接口示意：

```python
class AuthProvider(Protocol):
    async def resolve_identity(self, context) -> ExternalIdentity: ...
    async def get_invocation_credential(self, context) -> InvocationCredential: ...
    async def refresh(self, context) -> InvocationCredential: ...
    async def revoke(self, context) -> None: ...
```
...
NoAuthProvider
ProjectSessionAuthProvider
```

MSS 历史项目中的“以真实用户静默登录并复用External Session”只作为：

```text
MSSSessionAuthProvider
```

存在于 MSS Integration，不进入 Framework Core。

关键约束：

- Credential 明文只从 Secret Provider 获取；
- External Session 不保存在 Agent Runtime/Worker Pod 本地作为 SoT；
- Console 可配置/测试 AuthProfile，但 Runtime 不依赖 Console 进程实时在线；
- LLM 不得提供或修改 actor/auth identity；
- 外部系统返回 unauthorized/forbidden 必须作为真实授权失败，不允许 Agent 绕过。


### 4.1.8 Channel Gateway

Channel Gateway 是统一消息通道服务，独立部署。
...
```text
Agent realtime response / Worker DeliveryCommand
→ Channel Gateway
→ Adapter
→ Target Channel
```
...
### ER 图 A：用户、通道、Agent、Service、Integration 与能力配置
...
erDiagram
    PLATFORM_USER {
        uuid id PK
        string tenant_id
        string external_platform_user_ref
        string display_name
        string status
        datetime created_at
    }
    ...
    EXTERNAL_AUTH_PROFILE {
        ...
        string external_session_ref
        datetime external_session_expires_at
        string status
    }
    ...
    AGENT_DEFINITION ||--o{ CHANNEL_ACCOUNT : default_agent

    PLATFORM_USER ||--o{ EXTERNAL_AUTH_PROFILE : has_auth_profile
    PLATFORM_USER ||--o{ USER_MEMORY : owns
    PLATFORM_USER ||--o{ USER_SERVICE_AUTH : granted
    SERVICE_DEFINITION ||--o{ USER_SERVICE_AUTH : authorizes
...
> 说明：Agent 与 Skill / Knowledge / Capability / Service 的多对多绑定，在物理数据库中使用轻量 binding 表实现，例如 `agent_skill_binding`、`agent_knowledge_binding`、`agent_capability_binding`、`agent_service_binding`。为避免总体 ER 图过密，上图不逐一展开 binding 表字段。发布时由 Agent/Service 的 Published Snapshot 记录最终解析后的绑定集合。
...
```text
channel_identity:
UNIQUE(channel_account_id, peer_type, peer_id)

channel_binding:
同一个 channel_identity 同时最多一个 ACTIVE binding

external_auth_profile:
UNIQUE(platform_user_id, provider_key)

user_service_auth:
UNIQUE(platform_user_id, service_id, resource_scope_ref)

capability_definition:
UNIQUE(name)

service_release:
UNIQUE(service_id, release_id)
...
### ChannelAccount / ChannelIdentity / ChannelBinding
...
`channel_account` 表示一个可连接的 Channel 账号/端点，例如一个 WeCom Bot。`default_agent_id` 用于确定该 Channel 默认进入哪个已发布 Agent；未来如需更复杂的路由，可扩展独立 routing policy，而不是把路由写死在 Gateway 代码中。
...
`peer_id` 保留渠道原始值，不做 lowercase。

### UserAgentBinding（V1.7 D06 已删除）

V1 不建模该对象。用户到 Agent 的路由由 `ChannelAccount.default_agent + Channel Routing Policy` 决定；per-user 个性化未来单独建模。以下为删除前形态，仅留档：

```text
user_agent_binding（已删除）
- user_id
- agent_definition_id
- preference_revision
```
...
- external_session_expires_at
- status
- revision
```

真正 secret/session payload 使用 Secret Provider 或加密专用存储。
...
- memory_policy
- revision
- draft_payload(optional)
- published_payload(optional)
...
Draft/Test 与 Publish 使用同一 Skill Package artifact，只改变发布状态/引用版本，不用另一份简化脚本。
...
- id
- capability_id
- type(http/mcp/browser/project_sdk)
- config_ref
- adapter
- timeout_policy
- retry_policy
...
```text
service release_id/content_hash
agent release_id 或 revision
...
|---|---|---|---|
| 业务定义 | Service、对外独立 Agent | 草稿 → 已发布 | 是，内部 release/audit |
| 不可变资源 | Skill Package、静态知识文件 | 上传生成 immutable artifact + checksum | 是 |
| 运行配置 | ModelConfig、Capability Implementation、Knowledge Source、Channel Config、Memory Policy | 保存后直接生效 | 是，revision/audit |
...
当 Agent 是独立用户入口、或者其修改会直接影响在线用户时：
...
如果 Agent 只是某个 Service 内部使用的分析 Agent，则可以由 Service 发布时一起冻结其当前配置，不要求单独走发布流程。

Agent Definition 仍保持轻量：
...
其中 `revision` 是内部变更序号，不等于对管理员暴露的业务版本。

#### Skill：不可变 Artifact，不做复杂版本状态机
...
一旦上传的 artifact 被引用，就不原地修改。

更新 Skill 的方式是：

```text
上传新 artifact
→ Agent/Service 草稿选择新 artifact
→ 发布后生效
```
...
配置变更保存后直接生效；Execution 是否允许直接调用仍由 `side_effect + risk_level + execution_characteristic` 判断。
...
本地静态知识
外部知识库
未来 RAG / MCP Knowledge Provider
```

Endpoint、collection、top_k、过滤条件等运行配置默认直接生效。

外部知识库如果自身存在 revision/release，可以在检索记录中保留其 source revision；如果是实时变化的知识源，不要求平台复制整个知识库形成版本。
...
sequenceDiagram
    ...
    participant W as Channel Gateway
    participant A as Platform API
    participant DB as PostgreSQL

    U->>W: /bind CODE
    W->>A: channel/account/peer + code
    A->>DB: lookup binding code
    A->>A: validate code/user/status/expiry
    A->>DB: create ChannelIdentity + ChannelBinding
    A-->>W: platform_user_id + success
    W-->>U: 绑定成功
```
...
不会创建新 Pod，也不创建必须独占 Runtime 的 Agent 实例。

如果某 Channel 可提前获得稳定 `peer_id`，管理员可以预绑定；未知身份可采用一次性 Binding Code。MSS Reference Integration 中的 WeCom `userid` 只是该规则的一个实现。
...
例如 `shell.execute` 默认应为高风险且 `worker_preferred`；开发者可以为受控只读命令定义更窄的 Capability，而不是把一个无限制 shell 授权给普通 Agent。
...
静态 Skill Package 采用 immutable artifact/checksum，不建设复杂 Skill Version Center。
...
| Knowledge Runtime + SPI | 必须 | Framework package |
...
### Gate L：External Knowledge Provider
...
| Skill 作为 SOP 主要承载 | Service 为业务抽象，Skill 只承载 Agent 方法/知识 |
...
1. **Framework Core 与业务项目彻底解耦；MSS 是首个 Reference Integration，不是 Core 领域。**
...
8. **Capability、Knowledge、Auth、Channel 都以稳定 Contract/SPI 隔离项目实现。**
...
2. 《MSS 智能服务交付平台——用户场景与 Playbook 旅程设计 V5》。
