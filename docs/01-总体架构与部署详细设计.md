...
flowchart TB
    ...
    subgraph MUAD[MSS 智能服务交付平台]
      CP[console-platform\nControl Plane]
      IG[im-gateway\nIngress Plane]
      AR[agent-runtime\nRealtime Execution Plane]
      AW[agent-worker\nAsync Execution Plane]
      EB[Egress Boundary\n共享逻辑边界]
      PA[PlatformAdapter Registry\n认证与 Session 协议]
    end
...
- PlatformSession 可缓存在 Redis；Redis 丢失只触发重新认证，因此不属于业务权威事实。
...
V1.2 Worker 已进入当前架构，不再是 Phase 2 占位。
