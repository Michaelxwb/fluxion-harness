...
- `im-gateway` 直接从 Bot 快照读取明文 Secret（DB 存储，主键引用）；Console Internal API 返回该字段。
...
PostgreSQL、Redis、NFS/企业文件存储均作为外部依赖单独部署（密钥明文存于 PostgreSQL，随库备份管理），不放入应用 Compose 作为生产拓扑。
