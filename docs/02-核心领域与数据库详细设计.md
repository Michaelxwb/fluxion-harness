...
erDiagram
    ...
    RUN_RECORD ||--o| RUNTIME_SNAPSHOT : freezes
    ...
    DELIVERY_ROUTE ||--o{ TASK_SCHEDULE : delivers_to
    PLATFORM_USER ||--o{ CONFIG_AUDIT_LOG : performs
```
...
| `secret` | text | NOT NULL | Bot Secret（明文） |
...
| `mcp_catalog_json` | jsonb | NOT NULL | MCP catalog revision/hash/tool definitions 快照 |
...
- 错过触发不补发（V1 仅 SKIP 语义），记审计事件与 `scheduled_misfire_total` 计数；ONCE 触发成功后 `status=COMPLETED`、写 `completed_at`，不再计算 `next_fire_at`。
...
>
> 并发与租约：同一 conversation 同时最多一个非终态 Run（数据库 partial unique index 约束）；`run_record` 持有 `lease_owner/lease_until/heartbeat_at`，执行 Pod 续租；终态写入必须 CAS；Runtime Reaper 将 lease 过期仍为 RUNNING 的 Run 置为 `FAILED(RUN_ABANDONED)`，会话随之解锁。`cancel_requested` 只表达取消请求，终态迁移仍按状态机由执行者或接收 Pod 完成。
...
delivery:dedupe:{delivery_key}         TTL 7d
...
1. Alembic 统一管理 `control/runtime/task` 业务表；`langgraph` schema 由初始迁移创建，其表由 LangGraph Checkpointer adapter 的 setup/migration 负责；
