...
| v1.3 | 2026-09-10 | 补「归属」列；S-03/S-04 的后置 E2E 段登记承接方（模块 06 / 05） |
...
| S-05 | FEAT-04 | P0 | integration | Manifest → ServiceRelease → ExecutionSnapshot | 本模块 | 已完成基础配置 | Service payload 声明 scope type + schema_hash，Proposal 带合法 scope | 合法创建成功 |
...
> | S-05 | Manifest 装载段 | `13-Project Integration 与 Registry.md` | `S-04`（Manifest → Registry） |
