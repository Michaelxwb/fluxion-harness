...
| RULE-06 | 系统约束 | `user_agent_binding` 已删除（V1.7 D06）；路由由 ChannelAccount.default_agent + Routing Policy 决定。 | — |
...
| S-05 | FEAT-04 | P0 | integration | Manifest → ServiceRelease → ExecutionSnapshot | 已完成基础配置 | Service payload 声明 scope type + schema_hash，Proposal 带合法/非法 scope | 合法创建成功；未知 type 拒（SERVICE_CONFIGURATION_INVALID），见 REQ-EXEC-001 |
...
| E-02 | FEAT-02 | integration | Repository → Published Snapshot | 尝试原地修改 published payload | 拒绝修改，要求建立 Draft 或新 published snapshot | 返回可识别错误，不泄露内部细节 |
...
|---|---|
| Domain | 对象语义、枚举、不变量 |
| Application | 发布、解析、校验用例 |
| Repository | 持久化接口 |
| Adapter | SQLAlchemy/PostgreSQL 实现 |
| Integration | 只组合/引用 Core 对象，不反向修改 Core |
...
监控发布失败次数、resolve 失败、无效 binding、配置 hash 不一致；阈值待定。
...
|---|---|---|---|---|---|
| 总体设计 V1.7 | FEAT-01 | LIB-02 | S-01, E-01 | integration/E2E | 已实现（unit；E2E 待联调） |
| 总体设计 V1.7 | FEAT-02 | LIB-01, LIB-03 | S-02, E-02 | integration/E2E | 已实现（unit+smoke；E2E 待联调） |
| 总体设计 V1.6 | FEAT-03 | 内部契约 | S-04 | integration/E2E | 已实现（模型层） |
| 总体设计 V1.7 | FEAT-04 | LIB-03 | S-03, S-05 | integration/E2E | 已实现（unit+smoke；E2E 待联调） |
...
|---|---|---|---|---|---|---|
| framework#RULE-ARCH-001 | required | Core 领域不包含项目对象 | §3.2/§3.3 | S-01/E-01 | `test_core_purity.py` | applied |
| framework#RULE-DB-001 | required | 所有自建表含三公共字段 | §3.3 | S-01 | `test_database_common_fields.py` | applied |
