...
| v0.2 | 2026-09-10 | 补「归属」列；补 1.2 修订历史（对齐 design-lite 模板） |
...
|---|---|---|---|---|---|---|---|
| S-01 | FEAT-01 | P0 | integration | Repository → PostgreSQL | 本模块 | 创建 Execution 并重启进程 | 状态可从 PG 恢复 |
| S-02 | FEAT-02 | P0 | integration | Redis Down → PG Polling | 本模块 | 关闭 Redis | 可靠执行正确性不受破坏，仅实时性可能下降 |

**异常场景**

| 场景ID | 功能ID | 测试层级 | 关键真实边界 | 归属 | 触发条件 | 系统行为 |
|---|---|---|---|---|---|---|
| E-01 | FEAT-04 | integration | Secret Provider | 本模块 | secret ref 不存在 | 显式失败，不回退日志/配置明文 |
| E-02 | FEAT-03 | integration | Object Store | 本模块 | 上传失败 | Artifact 不标记为成功，Execution 可按策略重试/失败 |
...
| 接口ID | 接口/函数 | 形态 | 说明 |
|---|---|---|---|
| SPI-01 | Repository Protocol | 函数库 | 领域持久化 | 本模块 |
| SPI-02 | ArtifactStore.put/get | SPI | 对象存储 | 本模块 |
| SPI-03 | SecretProvider.resolve(ref) | SPI | Secret 获取 | 本模块 |
| SPI-04 | CoordinationClient.wake/cancel | SPI | 非权威协调 | 本模块 |
