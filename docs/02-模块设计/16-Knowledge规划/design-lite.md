...
> **范围声明（V1.14.2）**：Knowledge 是**规划项**（ADR-022）——本模块**无 DB 表、无接口、无 Provider、无 Console 菜单**。本文档只记录规划范围与放开条件；一旦设计冻结，必须**整章新增**（表/接口/Provider/前端页同时落），不允许先放"占位表/占位菜单/占位路由"。总设与前端不得出现 Knowledge Runtime、KnowledgeProvider 或 `/console/knowledge`。
