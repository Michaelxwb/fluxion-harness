...
| S-04-06 | FEAT-04-05 | E2E | 独立能力测试用户 | 无任何 Service，Builder/Admin 打开能力测试 | 请求 CAP-API-07 不含 service_id；Builder 只选自己；Admin 可选 END_USER；伪造他人/跨租户 ID 时提交 403，不调用 Provider |
...
| 类型 | `config` 字段 |
|---|---|
| PLATFORM_SERVICE | `project_platform_id`（必填）/ `service_key`（必填）/ `path`（必填）/ `method`（必填）/ `request_mapping` / `response_mapping` |
...
**凭据来源 `auth_mode`（实现层顶层字段，不属于 `config`）**：`auth_mode` 是 `implementation.auth_mode`（**顶层、必填、默认 `NONE`**，三值 `USER_PLATFORM` / `SHARED_SECRET` / `NONE`），与 `implementation_type`/`config`/`execution_policy` 并列，**不写进 `config`**；`config` 内出现 `auth_mode` 一律 400 `CAPABILITY_IMPLEMENTATION_INVALID`。表单上 `USER_PLATFORM` 仅 PLATFORM_SERVICE 可选，其他类型不显示该选项；PLATFORM_SERVICE 始终必填项目平台。`SHARED_SECRET` 时必填 `implementation.shared_secret_ref`（只写 Secret 引用）；NONE/USER_PLATFORM 不提交该键。编辑响应只返回 `shared_secret_configured`，不回显引用；编辑 SHARED_SECRET 时重新填写引用，禁止拿状态布尔值充当 Secret 引用。

**支持模式**：多选 `implementation.supported_execution_modes`（SYNC/ASYNC），默认 `["SYNC"]`，至少选择一项、不得重复；可选仅同步、仅异步或两者。该集合在 `CAP-API-06` Contract 原样投影供 Service 表单使用；能力测试为同步测试，仅含 SYNC 时可用，否则提示在 Service ASYNC 步骤测试。
...
| 输入 | ① 测试输入：按该能力 `input_schema` 渲染的 JSON 编辑器，Schema 非法的 JSON **就地报错、不发请求**；② 测试用户 `test_user_id`：**仅当**该能力的 `auth_mode=USER_PLATFORM`（平台用户认证；凭据来源唯一由实现层 `auth_mode` 表达，能力本身不再声明）时显示，默认当前登录用户（AUTH-API-01）；候选来自 CAP-API-07 `/api/v1/capabilities/{capability_id}/test-user-candidates`，Builder 仅自己、Admin 可选租户内 ACTIVE 用户；与 Service 无关，不构造 service_id；③ 结果模式 `result_mode`：`INLINE` / `SUMMARY`，默认跟随后端实现策略 |
...
| 读取测试用户候选 | `CAP-API-07` | `GET /api/v1/capabilities/{capability_id}/test-user-candidates` | Capability Runtime |
