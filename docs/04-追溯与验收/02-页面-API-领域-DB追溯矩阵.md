...
| 服务 Draft 编辑/校验/测试/发布/紧急启停 | SVC-API-04..07, SVC-API-10 | Service 与 Execution | `service_definition`（draft_payload/draft_revision/enabled）、`service_release`、`execution_snapshot`（source=TEST）、`service_execution`（execution_source=TEST） |
| 服务 Release 历史/详情 | SVC-API-08..09 | Service 与 Execution | `service_release` |
| 执行列表/详情/取消/重试 | EXE-API-01..04 | Service 与 Execution / Worker | `service_execution`, `execution_step`, `async_task_run`, `execution_command`, `task_progress_event`, `artifact` |
...
| 执行结果产物下载 | EXE-API-06 | Service 与 Execution / 存储与基础设施适配 | `artifact`（`artifact_id` 为对外身份；`object_ref` 不外泄） |
...
| 能力 | CAP-API-01..06 | Capability Runtime | `capability_definition`（含 side_effect 四值 / risk_level / `invocation_policy`）, `capability_implementation`（`auth_mode` = 凭据来源唯一声明） |
...
| 用户 / 项目平台认证 | CRED-API-01..04 | Auth 与项目平台 | `user_project_credential`（credential/session 到期分离） + Secret Provider |
