...
本文用于定义 智能服务交付平台 V2.0 的总体架构、模块边界、关键系统流程、核心数据模型、技术选型与 DFX 设计约束，并作为后续模块详细设计、研发任务拆分、测试验收与架构评审的基线。
...
- Egress Boundary；
- OpenTelemetry；
- PostgreSQL/Redis/Artifact Store（NFS-backed RWX PVC）外部依赖。
...
External State:
PostgreSQL / Redis / Artifact Store（NFS-backed RWX PVC） / OTel Backend
```
...
- **物理视图**：Kubernetes Deployment + 外部 PG/Redis/Artifact Store。
...
| ModelDefinition | key, protocol, model_id, base_url, api_key, params, revision, enabled | 模型配置；V1.3 当前仅 OPENAI 协议 |
...
| BotAccount | id, bot_id, secret, agent_id, enabled | IM 路由 |
...
| UserCredentialRef | user_id, platform_id, credential_json | 用户凭据引用 |
| SharedCredentialRef | platform_id, credential_json | 平台共享凭据引用；每平台 0..1，无 priority |
...
- 密钥明文存于业务表（主键引用），不得进入日志/审计/Snapshot/LLM/响应；
