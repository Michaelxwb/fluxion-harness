 ...
 *
 * 测量方式：jsdom 同步 render(/) 20 次采样取 P95。jsdom 为代理测量——
...
function measureDialogMount(): number {
...
describe("NFR-PERF-01 首屏基线（jsdom 代理测量）", () => {
  it(`对话框 mount P95（n=${SAMPLES}）≤ ${P95_LIMIT_MS}ms`, async () => {
    const samples: number[] = [];
    for (let i = 0; i < SAMPLES; i += 1) {
      samples.push(measureDialogMount());
    }
