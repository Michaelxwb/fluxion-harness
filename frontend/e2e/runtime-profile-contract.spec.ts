...
test("S-CFG-03 高级设置无无效参数可编辑入口且有兼容说明", async ({ page }) => {
  ...
  // 兼容状态说明可见。
  await expect(editor.getByText(/历史兼容字段，执行不读取/)).toBeVisible();
});

test("S-CFG-03 默认创建带版本戳并落盘 v1", async ({ page, request }) => {
  await page.goto(`/console/#/build/agents/${AGENT_ID}/edit?tab=advanced`);
  ...
  const response = await request.get("/api/v1/resources/runtime_profile/tenant-default");
  expect(response.ok(), await response.text()).toBeTruthy();
