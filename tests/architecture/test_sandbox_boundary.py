...
def test_agent_runtime_does_not_directly_invoke_subprocess():
...
def test_framework_core_does_not_implement_local_shell():
    ...
    for path in root.rglob("*.py"):
        ...
        if "create_subprocess_exec" in text or "subprocess.popen" in text or "os.system(" in text:
            violations.append(str(path.relative_to(root)))
    assert not violations, "Local shell implementation belongs under adapters/sandbox: " + ", ".join(violations)
